...
load("@niantic//bzl/assemble:nasm_assemble.bzl", "nasm_assemble")
...
genrule(
    ...
    cmd = select({
        "@niantic//bzl/conditions:32bit": "echo '#define SIZEOF_SIZE_T 4' >> $@",
        "@niantic//bzl/conditions:64bit": "echo '#define SIZEOF_SIZE_T 8' >> $@",
        "@niantic//conditions:default": "echo '#error libjpegturbo SIZEOF_SIZE_T unset in BUILD' >> $@",
    }),
)
...
genrule(
    name = "jconfig-hdrs",
    srcs = [
        "@niantic//third_party/libjpegturbo:jconfig",
    ],
    outs = [
        "jconfig.h",
        "jconfigint.h",
...
libjpegturbo_copts = select({
    ...
    "@niantic//bzl/conditions:android-arm": [
        "-D__ARM_NEON__",
        "-DNEON_INTRINSICS",
        "-march=armv7-a",
        "-mfpu=neon",
...
cc_library(
    ...
    deps = select({
        "@niantic//bzl/conditions:arm": [":simd_armv7a"],
        "@niantic//bzl/conditions:arm64": [":simd_armv8a"],
        "@niantic//bzl/conditions:windows": [":simd_win_x86_64"],
...
cc_library(
    ...
    copts = libjpegturbo_copts,
    deps = select({
        "@niantic//bzl/conditions:windows": [":simd-win_x86_64"],
        "//conditions:default": [],
    }),
)
