...
import {
  ...
  RingGeometry, Splat, HiderMaterial, Location, PolyhedronGeometry, TorusGeometry,
  Map as MapAttribute, MapTheme, MapPoint, Persistent, ImageTarget, VideoMaterial, VideoControls,
} from './components'
import type {
  ...
  Map as SceneMap,
  MapTheme as SceneMapTheme,
  MapPoint as SceneMapPoint,
...
import {MAP_THEME_DEFAULTS} from '../shared/map-constants'
...
const applyLocationComponent = (
  world: World, eid: Eid, location: DeepReadonly<SceneLocation> | null | undefined
) => {
  if (location) {
    Location.set(world, eid, {
      ...
      visualization: location.visualization,
    })
  } else {
...
const applyMapComponent = (
  world: World,
  eid: Eid,
  map: DeepReadonly<SceneMap> | null | undefined,
  graphIdToEid: Map<string, Eid>
) => {
  if (map) {
    MapAttribute.set(world, eid, {
      ...map,
      targetEntity: convertEntityReference(graphIdToEid, map.targetEntity?.id),
    })
  } else {
    MapAttribute.remove(world, eid)
  }
}

const applyMapThemeComponent = (
  world: World, eid: Eid, mapTheme: DeepReadonly<SceneMapTheme> | null | undefined
) => {
  if (mapTheme) {
    MapTheme.set(world, eid, {...MAP_THEME_DEFAULTS, ...mapTheme})
  } else {
    MapTheme.remove(world, eid)
  }
}

const applyMapPointComponent = (
  world: World,
  eid: Eid,
  mapPoint: DeepReadonly<SceneMapPoint> | null | undefined,
  graphIdToEid: Map<string, Eid>
) => {
  if (mapPoint) {
    MapPoint.set(world, eid, {
      ...mapPoint,
      targetEntity: convertEntityReference(graphIdToEid, mapPoint.targetEntity?.id),
    })
  } else {
    MapPoint.remove(world, eid)
  }
}
...
export {
  ...
  applyLocationComponent,
  applyMapComponent,
  applyMapThemeComponent,
  applyMapPointComponent,
