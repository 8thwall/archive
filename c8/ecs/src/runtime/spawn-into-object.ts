...
import {
  ...
  applyHiddenComponent, applyImageTargetComponent, applyLightComponent, applyLocationComponent,
  applyMapComponent, applyMapPointComponent, applyMapThemeComponent, applyMaterialComponent,
...
const spawnIntoObjectWithDiff = (
  ...
  const shouldUpdateLocation = !prevObject || !isEqual(e.location, prevObject.location)
  if (shouldUpdateLocation) {
    applyLocationComponent(world, eid, e.location)
  }

  const shouldUpdateMap = !prevObject || !isEqual(e.map, prevObject.map)
  if (shouldUpdateMap) {
    applyMapComponent(world, eid, e.map, graphIdToEid)
  }

  const shouldUpdateMapTheme = !prevObject || !isEqual(e.mapTheme, prevObject.mapTheme)
  if (shouldUpdateMapTheme) {
    applyMapThemeComponent(world, eid, e.mapTheme)
  }

  const shouldUpdateMapPoint = !prevObject || !isEqual(e.mapPoint, prevObject.mapPoint)
  if (shouldUpdateMapPoint) {
    applyMapPointComponent(world, eid, e.mapPoint, graphIdToEid)
  }
...
const updateInstanceProperties = (
  ...
  const shouldUpdateLocation = (e.location !== undefined || prevObject?.location !== undefined) &&
    (!prevObject || !isEqual(e.location, prevObject.location) ||
    (!deletions?.location && prevDeletions?.location))
  const shouldRemoveLocation = deletions?.location
  if (shouldUpdateLocation || shouldRemoveLocation) {
    applyLocationComponent(world, eid, shouldRemoveLocation ? undefined : e.location)
  }

  const shouldUpdateMap = (e.map !== undefined || prevObject?.map !== undefined) &&
    (!prevObject || !isEqual(e.map, prevObject.map) || (!deletions?.map && prevDeletions?.map))
  const shouldRemoveMap = deletions?.map
  if (shouldUpdateMap || shouldRemoveMap) {
    applyMapComponent(world, eid, shouldRemoveMap ? undefined : e.map, graphIdToEid)
  }

  const shouldUpdateMapTheme = (e.mapTheme !== undefined || prevObject?.mapTheme !== undefined) &&
    (!prevObject || !isEqual(e.mapTheme, prevObject.mapTheme) ||
    (!deletions?.mapTheme && prevDeletions?.mapTheme))
  const shouldRemoveMapTheme = deletions?.mapTheme
  if (shouldUpdateMapTheme || shouldRemoveMapTheme) {
    applyMapThemeComponent(world, eid, shouldRemoveMapTheme ? undefined : e.mapTheme)
  }

  const shouldUpdateMapPoint = (e.mapPoint !== undefined || prevObject?.mapPoint !== undefined) &&
    (!prevObject || !isEqual(e.mapPoint, prevObject.mapPoint) ||
    (!deletions?.mapPoint && prevDeletions?.mapPoint))
  const shouldRemoveMapPoint = deletions?.mapPoint
  if (shouldUpdateMapPoint || shouldRemoveMapPoint) {
    applyMapPointComponent(world, eid, shouldRemoveMapPoint ? undefined : e.mapPoint, graphIdToEid)
  }
