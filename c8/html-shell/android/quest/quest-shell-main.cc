...
#include "c8/html-shell/android/quest/shell-app-state.h"
...
constexpr int NODE_THREAD_PRIORITY = -20;

/**
 * Process the next main command.
 */
void appCmd(struct android_app *app, int32_t cmd) {
  ShellAppState *appState = (ShellAppState *)app->userData;
  ...
  switch (cmd) {
    ...
    case APP_CMD_INIT_WINDOW: {
      ...
      // Create a pipe for sending input events from the UI thread to the Node thread.
      if (pipe(appState->inputEventPipeFd)) {
        C8Log("[quest-shell-main] Failed to create input event pipe");
        return;
      }
      ...
      appState->nodeThread = std::thread([appState, app]() {
        pthread_setname_np(pthread_self(), "NAE:NodeThread");
        int currentPriority = getpriority(PRIO_PROCESS, gettid());
        int ret = setpriority(PRIO_PROCESS, gettid(), NODE_THREAD_PRIORITY);
        if (ret != 0) {
          C8Log("[quest-shell-main] Failed to set thread priority: %d", NODE_THREAD_PRIORITY);
        }
        Vector<String> args = {
          "node",
...
#endif
          // v8 has the fastest speed with TURBO_FAN. Use these check that our code is optimized.
          // "--trace-opt",
          // "--trace-deopt",
          ...
          appState->filesDir + "/app.js",  // The app's main script, expected to be the last arg.
        };
        ...
        // Start Node.
        NodeBinding::onCreate(
          argc,
          argv,
          appState->filesDir.c_str(),
          appState->nativeWindow,
          appState->windowWidth,
          appState->windowHeight,
          appState->inputEventPipeFd[0],  // Read end of the pipe.
          appState->appUrl.c_str(),
          appState->niaEnvironmentAccessCode.c_str(),
          appState->systemLocale.c_str(),
          app);
      });
      break;
    }
...
static int32_t handleInput(struct android_app *app, AInputEvent *event) {
  ...
  InputEventData inputEventData = convertFromPlatformEvent(event);
...
void android_main(struct android_app *app) {
  try {
    ...
    if (appState.appUrl.empty()) {
      C8Log("[quest-shell-main] No app URL found in manifest or intent. Ending app.");
      std::terminate();
    }
    ...
    for (const auto &asset : assetCacheFiles) {
      ...
      if (!fileExists(destPath.c_str())) {
        // Assets can be large, so only copy them over if they don't already exist.
        copyAssetFile(assetManager, asset.c_str(), destPath.c_str());
      }
    }

// App ID from the Oculus Developer Dashboard.
#if defined(NAE_OPT) && defined(OCULUS_APP_ID)
    if (
      ovr_PlatformInitializeAndroid(OCULUS_APP_ID, app->activity->clazz, env)
      != ovrPlatformInitialize_Success) {
      C8Log("[quest-shell-main] Failed to initialize Oculus Platform.");
      std::terminate();
    }
    ...
    while (!app->destroyRequested) {
      ...
      if (result == ALOOPER_POLL_ERROR) {
        C8Log("[quest-shell-main] ALooper_pollOnce returned an error");
        std::terminate();
      }
    ...
    C8Log("[quest-shell-main] Unknown Error");
  }
  // NOTE(paris): This is a fix for a somewhat tricky issue. When we quit the app (i.e. after
  // APP_CMD_DESTROY) without ths change, we get the following exception:
  //  android.app.IntentReceiverLeaked: Activity android.app.NativeActivity has leaked
  //  IntentReceiver com.oculus.vrapi.SystemActivitiesReceiver@e1f617 that was originally registered
  //  here. Are you missing a call to unregisterReceiver()?
  // The result of this exception is that when you next open the app, it will use the same
  // process. This leads to a crash in node, as it expects to only be initialized once per process.
  // The correct fix is to unregister the receiver, but, importantly, even with that, we may still
  // want to keep this. This is because the destroy doesn't destroy the app, but the app's activity.
  // And after the activity is destroyed, Android will keep the process around in case it can be
  // restarted.
