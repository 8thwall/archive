...
PACKAGE="com.the8thwall.catchthestack"
...
bazel build \
  ...
  --config=angle \
  ${BAZEL_SERIAL} \
  //apps/client/nae/catch-the-stack/android:catch-the-stack
trap '{ adb ${SERIAL} shell am force-stop ${PACKAGE}; exit 1; } ' INT
adb ${SERIAL} logcat -c
adb ${SERIAL} install -d -r bazel-bin/apps/client/nae/catch-the-stack/android/catch-the-stack.apk
