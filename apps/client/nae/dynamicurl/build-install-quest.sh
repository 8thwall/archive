...
PACKAGE="com.the8thwall.dynamicurl.quest"
...
bazel build \
  ...
  ${BAZEL_SERIAL} \
  //apps/client/nae/dynamicurl:dynamicurl-quest
trap '{ adb ${SERIAL} shell am force-stop ${PACKAGE}; exit 1; } ' INT
adb ${SERIAL} logcat -c
adb ${SERIAL} install -d -r bazel-bin/apps/client/nae/dynamicurl/dynamicurl-quest.apk
