...
AFRAME.registerComponent('rover', {
  init: function() {
    ...
    this.base.id = 'base'
    this.base.object3D.position.y = -1
    this.base.object3D.scale.set(0.05, 0.05, 0.05)
...
AFRAME.registerComponent('rover-scene', {
  init: function() {
    ...
    const groundShape = new AMMO.btBoxShape(new AMMO.btVector3(groundSize, 4, groundSize))
    const ground = document.createElement('a-box')
    ground.setAttribute('material', 'opacity: 0.0')
    ground.object3D.scale.set(groundSize * 2, 8, groundSize * 2)
    ground.object3D.position.z = 3 - groundSize * 1
    ground.object3D.position.y = -4
    ...
    hill.object3D.position.set(9, 0, -17)
    hill.object3D.scale.set(235, 235, 235)
    hill.addEventListener('model-loaded', () => {
      const geometry = hill.getObject3D('mesh').children[0].children[0].geometry
    ...
    hill2.addEventListener('model-loaded', () => {
      const geometry = hill2.getObject3D('mesh').children[0].children[0].geometry
    ...
    hill3.addEventListener('model-loaded', () => {
      const geometry = hill3.getObject3D('mesh').children[0].children[0].geometry
    ...
    this.fragments = [
      {
        position: new THREE.Vector3(0, 7, 0).add(hill.object3D.position),
      },
      {
        position: new THREE.Vector3(-1, 2, -5).add(flag.object3D.position),
      },
      {
        position: new THREE.Vector3(-5, 7.5, 3).add(hill3.object3D.position),
      }
    ]
    ...
    for (let i = 0; i < this.fragments.length; i++) {
      ...
      fragmentElement.setAttribute('gltf-model', `#fragmentModel${i + 1}`)
      fragmentElement.object3D.scale.set(50, 50, 50)
...
const wheelRadiusFront = .7
const wheelWidthFront = .7

const wheelRadiusBack = .7
const wheelWidthBack = .7
...
AFRAME.registerComponent('vehicle', {
  init: function() {
    ...
    const physicsWorld = baseEl.components['rover-scene'].physicsWorld
...
AFRAME.registerComponent('vehicle-monitor', {
  schema: {
  ...
  init: function() {
    ...
    this.getChassis = () => {
      return this.el.object3D.userData.physicsBody
    }

    this.isOutOfBounds = () => {
      return this.el.object3D.position.distanceToSquared(this.data.boundsCenter) > this.boundsRadiusSquared
    }
    ...
    this.isStopped = () => {
      return this.getVehicle().getCurrentSpeedKmHour() < 1
    }
