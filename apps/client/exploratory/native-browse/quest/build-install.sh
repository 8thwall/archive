...
PACKAGE="com.nianticlabs.apps.client.exploratory.nativebrowse.quest"
...
bazel build \
  ...
  --define=NAE_INSPECTOR=true \
  ${BAZEL_SERIAL} \
  //apps/client/exploratory/native-browse/quest:native-browse
trap '{ adb ${SERIAL} shell am force-stop ${PACKAGE}; exit 1; } ' INT
adb ${SERIAL} logcat -c
adb ${SERIAL} install -d -r bazel-bin/apps/client/exploratory/native-browse/quest/native-browse.apk
