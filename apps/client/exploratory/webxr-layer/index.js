...
const snellenUrl = './textures/snellen.png'
const maryOculusUrl = './textures/MaryOculus.webm'
...
//
// The background of the scene is an equirectangular layer. It uses an
// XRMediaBinding to render the contents of a video element into the scene. This
// example uses a low resolution video to avoid large files, but using media
// layers allows video that is higher resolution than normal rendering.
...
snellenConfig.quadWidth = 0.5 * snellenConfig.widthMeters / snellenConfig.cropX
snellenConfig.quadHeight = 0.5 * snellenConfig.heightMeters / snellenConfig.cropY
...
function init() {
  ...
  // The invisible dummyMesh quads and the guiMesh need to be rendered before
  // the controller lines so that they leave a hole in the depth buffer that the
  // lines can intersect.
  line.renderOrder = 1
  ...
  scene.add(eyeCharts)
  snellenTexture = new THREE.TextureLoader().load(snellenUrl)
...
function animate(t, frame) {
  ...
  if (session && guiLayer && (guiLayer.needsRedraw || guiLayer.needsUpdate)) {
    ...
    gl.texSubImage2D(
      gl.TEXTURE_2D, 0, 0, 0, canvas.width, canvas.height, gl.RGBA, gl.UNSIGNED_BYTE, canvas
    )
