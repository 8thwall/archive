...
class GLTFMeshGpuInstancing {
  ...
  createNodeMesh(nodeIndex) {
    ...
    return Promise.all(pending).then((results) => {
      ...
      const {count} = results[0]  // All attribute counts should be same
...
class GLTFDracoMeshCompressionExtension {
  ...
  decodePrimitive(primitive, parser) {
    ...
    return parser.getDependency('bufferView', bufferViewIndex).then(bufferView => new Promise((resolve, reject) => {
...
class GLTFParser {
  ...
  parse(onLoad, onError) {
    ...
    Promise.all(this._invokeAll(ext => ext.beforeRoot && ext.beforeRoot())).then(() => Promise.all([
      ...
      return Promise.all(parser._invokeAll(ext => ext.afterRoot && ext.afterRoot(result))).then(() => {
  ...
  getDependency(type, index) {
    ...
    if (!dependency) {
      switch (type) {
        ...
        case 'node':
          dependency = this._invokeOne(ext => ext.loadNode && ext.loadNode(index))
          break
  ...
  loadMaterial(materialIndex) {
    ...
    if (materialDef.emissiveFactor !== undefined && materialType !== MeshBasicMaterial) {
      const {emissiveFactor} = materialDef
