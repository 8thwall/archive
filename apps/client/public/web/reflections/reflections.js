...
const reflectionsComponent = {
  ...
  setReflections() {
    if (this.data.type === 'realtime') {
      ...
      mesh ? applyRealtimeEnvMap(mesh, cubeCamera.renderTarget.texture) : this.el.sceneEl.addEventListener('realityready', () => {
        applyRealtimeEnvMap(this.el.getObject3D('mesh'), cubeCamera.renderTarget.texture)
      })
      ...
      const mesh = this.el.getObject3D('mesh')
      mesh ? applyStaticEnvMap(mesh, data.materials, this.texture, data.reflectivity) : this.el.addEventListener('model-loaded', () => {
        applyStaticEnvMap(this.el.getObject3D('mesh'), data.materials, this.texture, data.reflectivity)
      })
    }
  },
  init() {
  ...
  update(oldData) {
    if (this.data.type === 'static') {
      ...
      if (addedMaterialNames.length) {
        applyStaticEnvMap(mesh, addedMaterialNames, this.texture, data.reflectivity)
      }
      ...
      if (oldData.materials && data.reflectivity !== oldData.reflectivity) {
        ...
        if (maintainedMaterialNames.length) {
          applyStaticEnvMap(mesh, maintainedMaterialNames, this.texture, data.reflectivity)
        }
      }
