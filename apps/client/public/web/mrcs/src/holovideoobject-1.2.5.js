...
class HoloVideoObject {
  ...
  _loadNextVideo() {
    ...
    if ((this.openOptions.streamMode == HoloVideoObject.StreamMode.HLS) ||
          (this.openOptions.streamMode == HoloVideoObject.StreamMode.Automatic && (this.isSafari && imageExt.hlsUri))) {
      video.src = this.urlRoot + imageExt.hlsUri
      video.mp4Name = imageExt.hlsUri
    } else if ((this.openOptions.streamMode == HoloVideoObject.StreamMode.Dash) ||
          (this.openOptions.streamMode == HoloVideoObject.StreamMode.Automatic && (!this.isSafari && imageExt.dashUri && typeof dashjs !== 'undefined'))) {
      if (!this.dashPlayer) {
    ...
    video.suspend = function () {
      ...
      this.videoState = HoloVideoObject.VideoStates.Suspended
    }.bind(this)
  ...
  play() {
    if (this.isSafari) {
      this.videoElements[this.currentVideoIndex].pause()  // Calling this now prevents a stalled video
...
if (typeof THREE !== 'undefined') {
  HoloVideoObjectThreeJS = class {
    ...
    constructor(renderer, callback, createOptions, updateCurrentFrameCallback) {
      ...
      hvo.onLoaded = function (fileInfo) {
        ...
        if (this.mesh) {
          ...
          // When this feature: https://github.com/mrdoob/three.js/pull/13196 lands in an official three.js release
          // this code will need to be updated to not rely on our patched access to renderer.geometries and renderer.attributes,
          // and instead setup our THREE.Mesh using GLBufferAttribute objects we create here.

          // Note from Josh S, July 2020:
          // This code has been updated to use the GLBufferAttribute functionality described above. However, the fork that
          // was to introduce this functionality to THREE.JS has not, at this time, been accepted. There is a patch,
          // WebGLAttributes.patch that must be applied to the source for r118, then r118 must be built, in order for this
      ...
      canvas.addEventListener('webglcontextrestored', (event) => {
        if (this.mesh) {
          ...
          uvBuf = bufferGeometry.attributes.uv.buffer
          indexBuf = bufferGeometry.getIndex().buffer
    ...
    /**
       ...
       * @param {HoloVideoObject.StreamMode} options.streamMode - Force the streamMode for the video object. Default is Automatic. Used for debugging only.
...
HoloVideoObject.StreamMode = {
  //* * Default: Selects based on browser */
  Automatic: 0,
  //* * Force load of full mp4 before playing */
  MP4: 1,
  //* * Force use of HLS for playback. Usually used only in Safari */
  HLS: 2,
  //* * Force use of Dash for segmenting video and faster load. Usually NOT used in Safari. Requires Dash player. */
  Dash: 3,
}
...
HoloVideoObject.Version.Patch = 5
HoloVideoObject.Version.String = `${HoloVideoObject.Version.Major}.${HoloVideoObject.Version.Minor}.${HoloVideoObject.Version.Patch}`

window.HoloVideoObjectThreeJS = HoloVideoObjectThreeJS
window.HoloVideoObject = HoloVideoObject
