...
const aframeComponent = () => ({
  ...
  init() {
    ...
    module.listeners.push(
      {
        event: 'hand-coaching-overlay.show',
        process: () => {
          this.el.sceneEl.emit('hand-coaching-overlay.show')
        },
      },
      {
        event: 'hand-coaching-overlay.hide',
        process: () => {
          this.el.sceneEl.emit('hand-coaching-overlay.hide')
        },
      }
    )
