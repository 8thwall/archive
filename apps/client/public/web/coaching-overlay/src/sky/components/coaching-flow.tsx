...
const CoachingFlow: FC<CoachingOverlayUiConfig> = (
  ...
  useEffect(() => {
    ...
    if (shouldShow) {
      setVisible(true)
      setClassNames('sky-coaching-overlay-fade-in')
    } else {
      setClassNames('sky-coaching-overlay-fade-out')
    }
  ...
  if (!disablePrompt && visible) {
    return (
      <div className='sky-coaching-overlay-container'>
        <div className={`sky-coaching-overlay-prompt-container ${classNames}`}>
          <MovementAnimation animationColor={animationColor} />
          <p className='sky-coaching-overlay-prompt-status' style={{'color': promptColor}}>
            {promptText}
          </p>
        </div>
      </div>
