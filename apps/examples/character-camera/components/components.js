...
const shareNameComponent = {
  ...
  init() {
    ...
    const stopEditing = () => {
      ...
      copyToClipboard(`https://playground.8thwall.app/character-camera/?name=${currentText}`)
  ...
  init() {
    ...
    el.addEventListener('animation-loop', (event) => {
      if (event.detail.action._clip.name === 'dance' && canStop) {
        el.emit('tapend', '')
        el.setAttribute('animation-mixer', 'clip: idle;loop:repeat;crossFadeDuration: 0.35;')
    ...
    this.el.addEventListener('click', () => {
      ...
      el.setAttribute('animation-mixer', 'clip:dance;loop:none;crossFadeDuration: 0.35;')
    })
  },
}
