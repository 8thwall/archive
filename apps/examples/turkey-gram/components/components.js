const shareNameComponent = {
  ...
  init() {
    ...
    message.textContent = `Happy Thanksgiving, ${pName}!`
    ...
    const stopEditing = () => {
      ...
      copyToClipboard(`https://8w.8thwall.app/turkey-gram/?name=${currentText}`)

      const shareData = {
        url: `https://8w.8thwall.app/turkey-gram/?name=${currentText}`,
      }
...
const clickMoveComponent = {
  ...
  init() {
    ...
    const updatePoseNames = () => {
      if (this.el.object3D.children[0] && this.el.object3D.children[0].animations && this.el.object3D.children[0].animations.length >= 2) {
    ...
    if (!this.tapRing) {
      ...
      tapRingInner.setAttribute('emissive', '#FF9500')
    ...
    this._groundClickHandler = (event) => {
      ...
      const touchPosition = new THREE.Vector3(event.detail.intersection.point.x, 0, event.detail.intersection.point.z)
...
const characterTapComponent = {
  init() {
    ...
    const hiSound = document.getElementById('excuseMeHi').components.sound
    let moving = false
    let canStop = false
    let waving = false

    link.addEventListener('move', () => moving = true)
    link.addEventListener('arrived', () => moving = false)

    link.addEventListener('animation-loop', (event) => {
      if (event.detail.action._clip.name === 'wave' && canStop) {
        link.emit('tapend', '')
        link.setAttribute('animation-mixer', 'clip: idle;loop:repeat')
        waving = false
        link.emit('freeze', '')
      }
    })
    ...
    this.el.addEventListener('click', () => {
      ...
      link.setAttribute('animation-mixer', 'clip:wave;loop:repeat')
    })
  },
}
...
const lookControllerComponent = {
  init() {
    ...
    el.addEventListener('look', (event) => {
      // console.log('[look-controller] looking')
      el.setAttribute('look-at-slerp', event.detail)
    })

    el.addEventListener('lookreset', () => {
      // console.log('[look-controller] reset')
      el.setAttribute('look-at-slerp', {target: 'camera', tilt: false, axis: '0 0 -1'})
    })
  },
}
...
const lookAtSlerpComponent = {
  ...
  update() {
    ...
    let hasTarget = false
    if (this.data.target) {
      const parsedTarget = AFRAME.utils.coordinates.parse(this.data.target)
      if (parsedTarget.x) {
    ...
    state.offsetVector.copy(AFRAME.utils.coordinates.parse(this.data.offset))
    state.targetOffsetVector.copy(AFRAME.utils.coordinates.parse(this.data.targetOffset))
    state.axisVector.copy(AFRAME.utils.coordinates.parse(this.data.axis)).normalize()
  ...
  tick(time, timeDelta) {
    ...
    if (state.updateLooking && state.hasTarget) {
      const offsetPosition =
        state.offsetVector.clone()
          .add(window.c8_getWorldPosition(this.el.object3D))

      let targetPosition
      if (state.targetObject) {
        targetPosition =
          state.targetOffsetVector.clone()
            .applyQuaternion(window.c8_getWorldQuaternion(state.targetObject))
      ...
      if (this.data.tilt) {
        const groundDistance =
              Math.pow(Math.pow(toTarget.x, 2) + Math.pow(toTarget.z, 2), 0.5)
        pitch = -Math.atan2(toTarget.y, groundDistance)
      }

      state.desiredRotation.setFromEuler(new THREE.Euler(pitch, yaw, 0, 'YXZ'))
    ...
    this.el.addEventListener('move', (event) => {
      ...
      switch (moveCommand.type) {
        case 'lerp':
          ...
          internalData.equation = p => startPosition.clone().lerp(moveCommand.endPosition, p)
          break
        case 'equation':
...
export {shareNameComponent, nameFromParamComponent, clickMoveComponent, characterTapComponent, lookControllerComponent, lookAtSlerpComponent, moveControllerComponent}
