// Copyright 2022 Arcturus Studios Holdings, Inc. All Rights Reserved.
...
const holostreamComponent = () => ({
  ...
  async init() {
    ...
    this.holoStream.sequenceReadyFunctions.push({
      update: () => {
        this.contentLoaded = true
      },
    })

    const openedHoloStream = await this.holoStream.openURL(this.data.src)
    if (!openedHoloStream) {
      console.error(`Could not open stream at location: ${this.data.src}`)
      return
    }
    ...
    this.placeHologram = (event) => {
      ...
      console.log(this.holoStream)
    }
  },
  tick() {
    // Display loading
    if (!this.showedPrompt && !this.contentLoaded) {
