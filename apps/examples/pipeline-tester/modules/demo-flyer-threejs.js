const flyerThreejsModule = () => {
  ...
  const frameEdge = (x, y, w1, h1, w2, h2) => {
    ...
    const big = new THREE.Mesh(new THREE.BoxGeometry(w1, h1, 0.008),
      new THREE.MeshBasicMaterial({color: DARK_GRAY}))
    big.position.set(x, y, 0)
    const small = new THREE.Mesh(new THREE.BoxGeometry(w2, h2, 0.012),
  ...
  const framePane = (scaledWidth, scaledHeight) => {
    ...
    material.alphaMap = new THREE.DataTexture(new Uint8Array([0, 127, 0, 0]), 1, 1, THREE.RGBAFormat)
    material.alphaMap.needsUpdate = true
    material.transparent = true
    return new THREE.Mesh(new THREE.BoxGeometry(scaledWidth, scaledHeight, 0.001), material)
  }
  ...
  const axis = () => {
    ...
    const cylinder = new THREE.CylinderGeometry(0.01, 0.01, axisLength, 32)
  ...
  const showImageFrame = ({detail}) => {
    ...
    frame.visible = true
    scaleP.innerText = `scale ${detail.scale}`
  ...
  const onStart = ({canvasWidth, canvasHeight}) => {
    ...
    scaleP = document.createElement('p')
    scaleP.style.position = 'absolute'
    scaleP.style.zIndex = 5000
    scaleP.style.top = '5em'
    scaleP.style.right = '0px'
    scaleP.style.padding = '1em'
    scaleP.style.margin = '1em'
    scaleP.style.background = 'white'
    scaleP.style.display = 'none'
    document.body.appendChild(scaleP)
  }
