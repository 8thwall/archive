...
const tapplacePipelineModule = () => {
  const modelFile = require('../assets/tapplace/tree.glb')      // 3D model to spawn at tap
  const startScale = new THREE.Vector3(0.0001, 0.0001, 0.0001)  // Initial scale value for our model
  const endScale = new THREE.Vector3(7.0, 7.0, 7.0)             // Ending scale value for our model
  ...
  const initXrScene = ({scene, camera}) => {
    ...
    scene.add(new THREE.AmbientLight(0x404040, 40))  // Add soft white light to the scene.
  ...
  const placeObject = (pointX, pointZ) => {
    loader.load(
      ...
      (xhr) => {
        console.log(`[tapplace-threejs] ${(xhr.loaded / xhr.total) * 100}% loaded`)
      },          // progress handler.
      (error) => {
        console.log('[tapplace-threejs@placeObject]', error)
      }           // error handler.
    )
  }
