...
const cubeThreejsPipelineModule = () => {
  const purple = new THREE.Color(0xAD50FF)
  const red = new THREE.Color('rgb(221, 0, 101)')
  let cube_ = null
  ...
  const initXrScene = ({scene, camera, renderer}) => {
    ...
    cube_ = new THREE.Mesh(new THREE.BoxGeometry(1, 1, 1), material)
    cube_.position.set(0, 0.5, -2)
  ...
  return {
    ...
    onUpdate: ({reality, processCpuResult}) => {
      // Color cube based on whether we're tracking or not.
      if (processCpuResult.reality && processCpuResult.reality.trackingStatus === 'NORMAL') {
        cube_.material.color = purple
      } else {
        cube_.material.color = red
      }
    },
  }
}
