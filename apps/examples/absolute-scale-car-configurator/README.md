...
![](https://media.giphy.com/media/22qce77aXgifSXpGz1/giphy.gif)
...
Creates a button carousel that changes the color of the car when clicked.
...
```cubemap-realtime``` (*/js/cubemap-realtime.js*)

This component generates a realtime cubemap from the camera feed texture and applies it as an environment map to a glb model.

```cubemap-static``` (*/js/cubemap-static.js*)

This component generates a static cubemap from a series of textures and applies it as an environment map to a glb model.

```xr-light``` (*/js/xr-light.js*)

This component lets ```xrweb``` adjust a light's intensity value to best match the camera feed.
