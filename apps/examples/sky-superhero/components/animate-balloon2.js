const animateBalloon2Component = {
  init() {
    let balloon
    ...
    const animate = () => {
      ...
      this.el.sceneEl.addEventListener('transition-end', (event) => {
        if (event.detail.id === 'balloon2') {
          ...
          balloon.setAttribute('id', 'newBalloon2')
      ...
      setTimeout(() => {
        this.el.setAttribute('transition-scene', {direction: 'skyToSlam', id: 'balloon2'})  // Time transition with animation speeds
      }, 9500)
...
export {animateBalloon2Component}
