...
class GrassGenerator {
  ...
  generateField(startRadius, endRadius) {
    const positions = []
    const uvs = []
    const indices = []
    const colors = []
    ...
    for (let i = 0; i < this.bladeCount; i++) {
      ...
      if (distanceFromCenter < startRadius || distanceFromCenter > endRadius) {
        continue  // Skip blades outside the specified range
      }
...
const GrassField = ecs.registerComponent({
  ...
  add: (world, component) => {
    ...
    grassGenerator.generateField(startRadius, endRadius)
