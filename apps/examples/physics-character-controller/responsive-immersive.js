  ...
  init() {
    const onAttach = ({sessionAttributes}) => {
      ...
      if (
        ...
        camera.setAttribute('rotation', '-35 0 0')

        // Hide the Jump Zone
        const jumpZone = document.getElementById('jumpZone')
        jumpZone.style.display = 'none'

        // Add the space bar instruction
        const spaceBarInstruction = document.createElement('div')
        spaceBarInstruction.id = 'spaceBarInstruction'
        spaceBarInstruction.textContent = 'Jump with Space Bar'
        document.body.appendChild(spaceBarInstruction)
      }
    }
    const onxrloaded = () => {
