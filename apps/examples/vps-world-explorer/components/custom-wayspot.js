...
const responsiveMapThemeComponent = {
  schema: {
    ...
    lightTheme: {type: 'string', default: 'natural'},
...
const mapDebugControlsComponent = {
  ...
  tick() {
    ...
    if (this.fwd && this.left) {  // NW
      ...
      if(this.char !== undefined) {this.char.setAttribute('rotation', '0 120 0')}
    }
    ...
    if (this.fwd && this.right) {  // NE
      ...
      if(this.char !== undefined) {this.char.setAttribute('rotation', '0 45 0')}
    }
    ...
    if (this.fwd && !this.left && !this.right) {  // N
      ...
      if(this.char !== undefined) {this.char.setAttribute('rotation', '0 90 0')}
    }
    ...
    if (this.left && !this.fwd && !this.back) {  // E
      ...
      if(this.char !== undefined) {this.char.setAttribute('rotation', '0 180 0')}
    }

    if (this.right && !this.fwd && !this.back) {  // W
      const minusLat = parseFloat(this.locArr[0]) - this.data.distance
      this.el.setAttribute('lat-lng', `${minusLat} ${this.locArr[1]}`)
      if(this.char !== undefined) {this.char.setAttribute('rotation', '0 0 0')}
    }
  },
}
