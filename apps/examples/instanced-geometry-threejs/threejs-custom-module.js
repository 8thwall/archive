...
//
// Custom ThreeJS based renderer

function CustomThreejsFactory() {
  let scene3
  let engaged = false
  let mycanvas = null
  let canvas_ = null
 
  function pipelineModule(mycanvas_) {
    mycanvas = mycanvas_
    if (!window.THREE) {
      throw new Error('window.THREE does not exist but is required by the ThreeJS pipeline module')
    }
    const engage = ({canvas, canvasWidth, canvasHeight}) => {
      if (engaged) {
        return
      }
      canvas_ = canvas
      mycanvas.style = canvas.style
      ...
      const renderer = new window.THREE.WebGLRenderer({
        mycanvas,
        context: mycanvas.getContext('webgl', {alpha: true}),
        alpha: true,
        antialias: true,
      })
