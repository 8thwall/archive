...
ecs.registerComponent({
  ...
  schema: {
    ...
    // @label UI: Overlay
    overlay: ecs.eid,  // Entity ID for overlay to enable after loading
  },
  schemaDefaults: {
    endTimeout: 500,
    fadeDuration: 1000,
  ...
  stateMachine: ({world, eid, schemaAttribute}) => {
    ...
    const {loadingBar, loadingBarBg, loadingLabelText, overlay, endTimeout, fadeDuration} = schemaAttribute.get(eid)
    // Get entity handles for easy enable/disable
    ...
    // "done" state: instantly fill bar, fade out loading UI, then switch to overlay
    const done = ecs.defineState('done').onEnter(() => {
      ...
      // Disable the loading screen, enable overlay after fade completes
      world.time.setTimeout(() => {
        eidEnt.disable()
        overlayEnt.enable()
