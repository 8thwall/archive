...
const cubeEnvMapComponent = {
  ...
  init() {
    ...
    mesh ? applyEnvMap(mesh, data.materials, this.texture, data.reflectivity) : window.addEventListener('model-loaded', applyEnvMap(mesh, data.materials, this.texture, data.reflectivity))
  },
  update(oldData) {
    const {data} = this
    const mesh = this.el.getObject3D('mesh')
