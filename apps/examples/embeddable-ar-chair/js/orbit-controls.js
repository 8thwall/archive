...
const orbitControlsComponent = {
  ...
  update(oldData) {
    if (this.data.rotateTo) {
      const rotateToVec3 = new THREE.Vector3(this.data.rotateTo.x, this.data.rotateTo.y, this.data.rotateTo.z)
  ...
  /**
    ...
    */
  tick(t) {
    const render = this.data.enabled && this.isRunning ? this.updateView() : false
  ...
  panHorizontally(distance, objectMatrix) {
  ...
  panVertically(distance, objectMatrix) {
  ...
  pan(deltaX, deltaY) {  // deltaX and deltaY are in pixels; right and down are positive
  ...
  updateView(forceUpdate) {
    if (this.desiredPosition && this.state === this.STATE.ROTATE_TO) {
      ...
      const phiDiff = desiredSpherical.phi - this.spherical.phi
      const thetaDiff = desiredSpherical.theta - this.spherical.theta
      this.sphericalDelta.set(this.spherical.radius, phiDiff * this.data.rotateToSpeed, thetaDiff * this.data.rotateToSpeed)
    }

    const offset = new THREE.Vector3()

    const quat = new THREE.Quaternion().setFromUnitVectors(this.dolly.up, new THREE.Vector3(0, 1, 0))  // so camera.up is the orbit axis
