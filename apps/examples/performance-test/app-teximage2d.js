...
<h1>MediaRecorder + texImage2D Performance Test</h1>
...
const stats = {
  texImage: Statistics(),
  texImageWithConfigs: Statistics(),
  texImageFromBuffer: Statistics(),
...
const processFrame = (timestamp) => {
  if (video && video.videoWidth > 0) {
    ...
    // (c) Draw with texImage2D from a ArrayBufferView.
    // NOTE - we do this before a) and b) b/c the first gl call is slower than the others. Because
    // this is already much faster than a) and b), we still illustrate the issue.
    helperContext.drawImage(video, 0, 0)
    const imageData = helperContext.getImageData(0, 0, video.videoWidth, video.videoHeight)
    stats.texImageFromBuffer.start()
    gl3.texImage2D(gl3.TEXTURE_2D, 0, gl3.RGBA, video.videoWidth, video.videoHeight, 0, gl3.RGBA, gl3.UNSIGNED_BYTE, imageData.data)
    stats.texImageFromBuffer.finish()
    ...
    // (b) Draw with texImage2D from a HTMLVideoElement, set gl.UNPACK_PREMULTIPLY_ALPHA_WEBGL=true.
    stats.texImageWithConfigs.start()
    gl2.texImage2D(gl2.TEXTURE_2D, 0, gl2.RGBA, gl2.RGBA, gl2.UNSIGNED_BYTE, video)
    stats.texImageWithConfigs.finish()

    // Check gl error.
    checkGLError(gl1, 'gl1')
    checkGLError(gl2, 'gl2')
    checkGLError(gl3, 'gl3')

    document.querySelector('pre').innerHTML = `
      Timestamp: ${timestamp} 
      ---
      texImage2D:                                   ${(stats.texImage.average()).toFixed(3)} ms
      texImage2D + UNPACK_PREMULTIPLY_ALPHA_WEBGL:  ${(stats.texImageWithConfigs.average()).toFixed(3)} ms
      texImage2D (from ArrayBufferView):            ${(stats.texImageFromBuffer.average()).toFixed(3)} ms
      ---
