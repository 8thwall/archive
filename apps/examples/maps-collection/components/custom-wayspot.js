...
const customWayspotComponent = {
  schema: {
    ...
    anyWayspotColor: {type: 'string', default: '#57BFFF'},
  },
  init() {
    ...
    // each instance listens for the themeSwitched event to change wayspot colors
    scene.addEventListener('themeSwitched', (e) => {
      switch (e.detail.theme) {
        case '8w dark':
          this.data.anyWayspotColor = '#57BFFF'
          break
        case 'nianticlight':
          this.data.anyWayspotColor = '#FFD167'
          break
        case 'natural':
          this.data.anyWayspotColor = '#B67CB3'
          break
        case 'vaporwave':
          this.data.anyWayspotColor = '#0EF096'
          break
        case 'violet sunset':
          this.data.anyWayspotColor = '#c1e8eb'
          break
        case 'bubblegum':
          this.data.anyWayspotColor = '#f4a4c0'
          break
        case 'home run':
          this.data.anyWayspotColor = '#ca4541'
          break
        case 'ice':
          this.data.anyWayspotColor = '#d9c9fe'
          break
        case 'roller rink':
          this.data.anyWayspotColor = '#fae561'
          break
        case 'lighttime':
          this.data.anyWayspotColor = '#628fee'
          break
        case 'mars':
          this.data.anyWayspotColor = '#503e3e'
          break
        case 'night lime':
          this.data.anyWayspotColor = '#b0ee4f'
          break
        case 'seaside':
          this.data.anyWayspotColor = '#93dbe5'
          break
        case 'swamp':
          this.data.anyWayspotColor = '#4c442a'
          break
        case 'newspaper':
          this.data.anyWayspotColor = '#000000'
          break
        case 'rust':
          this.data.anyWayspotColor = '#C6898A'
          break
        case 'muted':
          this.data.anyWayspotColor = '#6689AB'
          break
        case 'nighttime':
          this.data.anyWayspotColor = '#FCD070'
          break
        case 'trails':
          this.data.anyWayspotColor = '#8BDE69'
          break
        case 'arcade':
          this.data.anyWayspotColor = '#79DEF4'
          break
        default:
      }
      startArBtn.style.backgroundColor = this.data.anyWayspotColor  // set 'Start AR' button color
    ...
    this.el.appendChild(this.wayspotHolder) // add wayspot to scene

    this.el.emit('wayspotAdded', {color: this.data.anyWayspotColor}) // emit color of wayspot
