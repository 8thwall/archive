...
const WorldMapLocation = ecs.registerComponent({
  name: 'World Map Location',
  schema: {
  ...
  stateMachine: ({world, eid, schemaAttribute}) => {
    ...
    ecs.defineState('disc')
      ...
      .listen(eid, ecs.events.GLTF_MODEL_LOADED, (e) => {
        ...
        textureLoader.load(`${thumbnailImage}`, (tex: any) => {
          const object3d = world.three.entityToObject.get(eid)
          const outer = object3d.getObjectByName('outer')
          const inner = object3d.getObjectByName('inner')

          if (outer) {
            outer.material.color.setRGB(ringR / 255, ringG / 255, ringB / 255)
