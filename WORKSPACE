...
npm_package(
    name = "npm-c8",
    excluded_files = [
        "node_modules/tape/test/has spaces.js",
    ],
    package = "//bzl/npmpackage/c8:package.json",
    package_lock = "//bzl/npmpackage/c8:package-lock.json",
    patches = [
        "//bzl/npmpackage/c8/patches:node-canvas-webgl+0.2.6.patch",
    ],
)
...
npm_package(
    name = "npm-bzl-presubmit",  # For targets in //bzl/presubmit
    package = "//bzl/presubmit/npm:package.json",
    package_lock = "//bzl/presubmit/npm:package-lock.json",
)
...
new_git_repository(
    ...
    patches = [
        "@niantic//third_party/imgui:imconfig.h.patch",
        "@niantic//third_party/imgui:examples/example_apple_opengl2/main.mm.patch",
    ],
...
http_archive(
    ...
    patches = [
        "@niantic//third_party/mdnsresponder:dllmain.c.patch",
    ],
...
new_git_repository(
    ...
    patches = [
        "@niantic//third_party/libvpx:config.patch",
    ],
...
new_git_repository(
    ...
    patches = [
        "@niantic//third_party/fdk-aac:FDK_archdef.h.patch",
        "@niantic//third_party/fdk-aac:FDK_lpp_tran.cpp.patch",
    ],
...
rust_register_toolchains(
    ...
    # Skip registering toolchains - we instead register them ourselves and add a wrapper around
    # rustc which provides the correct SDKROOT.
...
load("//bzl/crosstool:rust.bzl", "rust_register_all_toolchains")

rust_register_all_toolchains("rust_toolchains")

load("@rules_rust//crate_universe:repositories.bzl", "crate_universe_dependencies")

crate_universe_dependencies()

# TODO(paris): We can use a single shared Cargo.lock file for all manifests instead of one tied to
# Tauri specifically.
# https://bazelbuild.github.io/rules_rust/crate_universe_workspace.html#cargo-workspaces
load("@rules_rust//crate_universe:defs.bzl", "crate", "crates_repository")

crates_repository(
    name = "tauri-deps",
    # NOTE(lreyna): Some dependencies are missing downstream resolutions for `aarch64-apple-darwin`,
    # we can get around those issues by using `crate.annotation` to specify the correct dependencies
    annotations = {
        "bytes": [crate.annotation(
            deps = ["@tauri-deps__serde-1.0.219//:serde"],
        )],
        "futures-util": [crate.annotation(
            proc_macro_deps = ["@tauri-deps__futures-macro-0.3.31//:futures_macro"],
        )],
        "ring": [crate.annotation(
            override_targets = {"lib": "@ring//:ring"},
        )],
        "openssl-sys": [crate.annotation(
            build_script_data = [
                "@openssl//:openssl-rust-dir",
            ],
            build_script_data_glob = ["build/**/*.c"],
            # Manually specify the OpenSSL library so that we use the hermetic Bazel version
            # instead of the host vesrion (Automatic mode) or a Rust-vendored version (Vendored mode):
            # - https://docs.rs/openssl/latest/openssl/#manual
            build_script_env = {
                "OPENSSL_STATIC": "1",
                "OPENSSL_DIR": "$(execpath @openssl//:openssl-rust-dir)",
            },
        )],
    },
    cargo_lockfile = "//c8/html-shell-tauri:Cargo.lock",
    isolated = True,
    lockfile = "//c8/html-shell-tauri:Cargo.Bazel.lock",
    manifests = ["//c8/html-shell-tauri:Cargo.toml"],
    # NOTE(paris): If you are testing updating rules_rust() by passing:
    # --override_repository=rules_rust=/Users/paris/repo/rules_rust, you may need to comment this
    # back in:
    # generator_urls = {
    #     "aarch64-apple-darwin": "https://github.com/bazelbuild/rules_rust/releases/download/0.57.1/cargo-bazel-aarch64-apple-darwin",
    # },
)

crates_repository(
    name = "ring-deps",
    cargo_lockfile = "//bzl/cargo/ring:Cargo.lock",
    isolated = True,
    lockfile = "//bzl/cargo/ring:Cargo.Bazel.lock",
    manifests = ["//bzl/cargo/ring:Cargo.toml"],
)

load("@tauri-deps//:defs.bzl", tauri_crate_repositories = "crate_repositories")

tauri_crate_repositories()

load("@ring-deps//:defs.bzl", ring_crate_repositories = "crate_repositories")

ring_crate_repositories()
...
http_archive(
    ...
    patch_args = ["-p1"],
    patches = [
        "@niantic//:third_party/spdlog/public_logger.patch",
    ],
