...
static void help() {
  cout << "\n------------------------------------------------------------------"
          "\n"
       << " This program shows the camera trajectory reconstruction "
          "capabilities\n"
       << " in the OpenCV Structure From Motion (SFM) module.\n"
       << " \n"
       << " Usage:\n"
       << "        example_sfm_trajectory_reconstruction <path_to_tracks_file> "
          "<f> <cx> <cy>\n"
       ...
       << "        etc\n"
       << " \n"
       << "        i.e. a row gives the 2D measured position of a point as it "
          "is tracked\n"
       << "        through frames 1 to 36.  If there is no match found in a "
          "view then x\n"
       << "        and y are -1.\n"
       << " \n"
...
int main(int argc, char **argv) {
  ...
  const double f = atof(argv[2]), cx = atof(argv[3]), cy = atof(argv[4]);
  ...
  cv::sfm::reconstruct(points2d, Rs_est, ts_est, K, points3d_estimated,
                       is_projective);
  ...
  if (path_est.size() > 0) {
    ...
    while (!window_est.wasStopped()) {
      ...
      for (size_t i = 0; i < point_cloud_est.size(); ++i) {
        ...
        viz::WCube cube_widget(Point3f(0.1, 0.1, 0.0), Point3f(0.0, 0.0, -0.1),
                               true, viz::Color::blue());
      ...
      viz::WCameraPosition cpw_frustum(K, 0.3,
                                       viz::Color::yellow()); // Camera frustum

      if (camera_pov)
        window_est.setViewerPose(cam_pose);
      else {
        // render complete trajectory
        window_est.showWidget("cameras_frames_and_lines_est",
                              viz::WTrajectory(path_est, viz::WTrajectory::PATH,
                                               1.0, viz::Color::green()));
