...
public:
  // Tally of objects.
  static int64_t globalObjects;
  static int64_t globalNew;
  static int64_t globalDelete;

  // Tally of outstanding references to those objects.
  static int64_t globalCount;
  static int64_t globalRetainCount;
  static int64_t globalReleaseCount;
  Counted() :
      count_(0) {
    ++globalObjects;
    ++globalNew;
  ...
  void release() {
    ...
    globalReleaseCount++;
    if (count_ < 0) {
      throw std::runtime_error("Releasing unretained object.");
    }
    if (count_ == 0) {
      count_ = 0xDEADF001;
      delete this;
