...
const { WebGLExport: { gl }, WebGL2Export: { NativeWebGL2RenderingContext } } = require('./native-gl')
...
class WebGLFramebuffer extends Linkable {
  constructor (_, ctx) {
    ...
    this._attachmentLayer[gl.COLOR_ATTACHMENT0] = 0
    this._attachmentLayer[gl.DEPTH_ATTACHMENT] = 0
    this._attachmentLayer[gl.STENCIL_ATTACHMENT] = 0
    ...
    } else if (ctx instanceof NativeWebGL2RenderingContext) {
      for (let i = 1; i < ctx._multipleRenderTargets._maxDrawBuffers; i++) {
        this._attachments[gl.COLOR_ATTACHMENT0 + i] = null
        this._attachmentLevel[gl.COLOR_ATTACHMENT0 + i] = 0
        this._attachmentFace[gl.COLOR_ATTACHMENT0 + i] = 0
      }
      this._attachments[gl.NONE] = null
      this._attachments[gl.BACK] = null

      this._attachmentLevel[gl.NONE] = 0
      this._attachmentLevel[gl.BACK] = 0

      this._attachmentFace[gl.NONE] = 0
      this._attachmentFace[gl.BACK] = 0
    }
  }
