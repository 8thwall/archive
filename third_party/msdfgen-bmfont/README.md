...
## This repo is a fork of [msdf-bmfont-xml](https://github.com/soimy/msdf-bmfont-xml/tree/master)

Forked in order to add support for MTSDF generation as well as creating an atlas of all glyphs that are contained within a `.ttf`
...
### Running without installing the module (Preferred)

To run without installing as a module or CLI, simply install all the required package by running

```bash
npm install
```

Then you can call `msdf-bmfont` from console through `npx` command

#### Generate mtsdf atlas for a specific font

`-u` generate a reusable config file and `-a` generates all glyphs.

```sh
npx msdf-bmfont -u -a path/to/your/font/file.ttf
```

### Install CLI globally (Not Preferred)
...
Options:
  ...
  -f, --output-type <format>    font file format: xml | json | txt (default: json)
  ...
  -m, --texture-size <w,h>      ouput texture atlas size (default: [512, 512])
  ...
  -t, --field-type <type>       msdf | sdf | psdf | mtsdf (default: mtsdf)
  -d, --round-decimal <digit>   rounded digits of the output font file. (default: 0)
  -v, --vector                  generate svg vector file for debuging (default: false)
  ...
  -a  --all-glyphs              generate all glyphs in charset (Default: false)
  -h, --help                    output usage information
```

### MTSDF

Currently, MTSDF generation is only supported for macs. If you'd like support
for your own OS, please compile the [Chlumsky/msdfgen project](https://github.com/Chlumsky/msdfgen)
and then point this tool to the binary that it should use to generate the atlas.

## CLI Examples

Generate a multi-channel signed distance field font atlas with ASCII charset,
font size 42, spread 3, maximum texture size 512x256, padding 1, and save out
config file:
...
If no charset is provided and all glyphs is false, will default to ASCII
...
- `outputType` (String)
  - type of output font file. Defaults to `json`
    ...
    - `txt`
- `filename` (String)
  - filename of both font file and font atlas. If omitted, font face name is
  used. **Required** if font is provided as a Buffer.
...
- `fieldType` (String)
  - what kind of distance field to generate. Defaults to `mtsdf`. Must be one of:
    - `msdf` Multi-channel signed distance field
    - `mtsdf` Multi-channel with True-distance signed distance field
...
- `vector` (Boolean)
  - output a SVG Vector file for debugging. Default: `false`
- `smart-size` (Boolean)
...
- `allGlyphs` (Boolean)
  - generate all glyphs in charset. Default: `false`
