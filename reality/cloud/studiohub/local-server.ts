...
import {guessIp} from '@nia/c8/cli/ip'
...
import {runBuildCommand, runProxyCommand} from './app/file-sync/run-commands'
...
const createLocalServer = async (savePath: string): Promise<LocalServer> => {
  await installBuildPackage(savePath)
  const webpackPort = await getPort({port: LOCAL_SERVER_PORT_RANGE})
  const webpackDevServer = runBuildCommand(savePath, webpackPort)

  const proxyPort = await getPort({port: LOCAL_SERVER_PORT_RANGE})
  const proxyProcess = runProxyCommand(savePath, proxyPort, webpackPort)
