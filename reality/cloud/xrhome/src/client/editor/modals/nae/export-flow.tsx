...
const ExportFlow: React.FC<IExportFlow> = ({onClose, platform}) => {
  ...
  React.useEffect(() => {
    ...
    if (
      (mostRecentBuild.refHead === 'master' && git.deployment.master) ||
      (mostRecentBuild.refHead === 'staging' && git.deployment.staging) ||
      (mostRecentBuild.refHead === 'production' && git.deployment.production)
    ) {
      setRef(mostRecentBuild.refHead)
    } else {
      setRef(activeRef)
    }
  }, [recentNaeBuilds])
