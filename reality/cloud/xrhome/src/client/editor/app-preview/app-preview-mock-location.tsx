...
const AppPreviewMockLocation: React.FC<IAppPreviewMockLocation> = (
  {locations, mapPoints, responsiveHeight, responsiveWidth, iframeRef}
) => {
  ...
  const handleCoordinateChange = (value) => {
    const selectedMockLocation = locations?.find(({poi}) => poi.id === value)
    const selectedMockMapPoint = mapPoints?.find(({id}) => id === value)

    updateSimulatorState({
      mockCoordinateValue: value,
      mockLat: selectedMockLocation?.poi?.lat ??
        selectedMockMapPoint?.mapPoint?.latitude ??
        mockLat,
      mockLng: selectedMockLocation?.poi?.lng ??
        selectedMockMapPoint?.mapPoint?.longitude ??
        mockLng,
    })
  }
  ...
  const locationOptions: CoordinateOption[] = locations?.map(({name, poi}) => ({
    value: poi.id,
    name,
    content: <DropdownOptionContent
      textContent={name}
      rightContent={poi?.address}
    />,
    section: 'LOCATIONS',
    poi,
  })) ?? []

  const mapOptions: CoordinateOption[] = mapPoints?.map(obj => ({
    value: obj.id,
    name: obj.name,
    content: <DropdownOptionContent
