...
import {getAssetSizeLimit, getTextSizeLimit} from '../../../shared/asset-size-limits'
import useActions from '../../common/use-actions'
import gitUploadActions, {
  getFileType, getFolderDest, UploadFileType,
} from '../../git/git-upload-actions'
import coreGitActions, {CreateFileOptions} from '../../git/core-git-actions'
...
import {serialize, type BundleAssetPointer} from '../../../shared/asset-pointer'
...
const useFileUploadState = (
  ...
  React.useEffect(() => {
    if (!Object.keys(fontStateRef.current).length) {
      return
    }
    Object.keys(fontStateRef.current).forEach(async (fileName) => {
      const state = conversionState[fileName]
      if (state.status === 'completed' && fontStateRef.current[fileName].isLoading === false) {
        try {
          const font8FileName = sanitizeFilePath(fileName).replace('.ttf', '.font8')
          fontStateRef.current[fileName].isLoading = true
          const {repoId, ttfFile} = fontStateRef.current[fileName]
          const repo = getRepoState({git: globalGit}, repoId)?.repo
          const filesToUpload = [ttfFile].concat(
            state.files.filter(file => file.name !== 'res.json')
          )
          const files = await Promise.all(filesToUpload.map(async (file) => {
            const sanitizedFilePath = sanitizeFilePath(file.name)
            const {file: assetPath} = await uploadActions.uploadAsset(
              repoId, repo, file, sanitizedFilePath, () => {}
            )
            return {filePath: sanitizedFilePath, assetPath: join('/', assetPath)}
          }))
          const {bundlePath} = await uploadActions.createAssetBundle(repo, repoId, files)
          const fileMap = files.reduce((acc, {filePath, assetPath}) => {
            acc[filePath] = assetPath
            return acc
          }, {})
          ...
          await createFile(
            repo, `assets/${font8FileName}`, serialize(pointer), {skipValidate: true}
          )
        } catch (err) {
          throw new Error(`Failed to upload font file ${fileName}: ${err}`)
        } finally {
          removeFileFromConversionState(fileName)
          delete fontStateRef.current[fileName]
        }
      }
    })
  }, [conversionState])
  ...
  const uploadFile = async (
    ...
    // Let the user know if the file is too big
    const fileLimit = filetype === UploadFileType.Asset
      ? getAssetSizeLimit(assetLimitOverrides)
      : getTextSizeLimit(assetLimitOverrides)

    if (file.size > fileLimit) {
      const errorMsg =
        `File too big: ${filetype} file "${file.name}" ` +
        `with size (${(file.size / (2 ** 20)).toFixed(2)}MB) ` +
        `is over the limit (${fileLimit / (2 ** 20)}MB)`
    ...
    await uploadActions.uploadFile(
      repo, repoId, file, filetype, sanitizedTargetPath, onProgress, options
    )
  }
  ...
  const uploadFiles = async (
    ...
    try {
      ...
      await Promise.all(filesAsArray.map(file => runQueue.next(async () => {
        ...
        if (file.name.endsWith('.ttf') && convertFont) {
          ...
          await uploadFile(file, folderPath, onProgress, options)
        }
        patchUploadState(s => ({
