...
import {getEditorTheme} from '../user/editor-settings'
...
import type {HcapInfo, ModelInfo} from '../editor/asset-preview-types'
import {Hcap} from './hcap-model'
...
const StudioThreeModelPreview: React.FunctionComponent<IModelPreview> = ({
  ...
  gridColor, showDebugUi = false, onModelInfo, onHcapInfo, children = null, isPublicView = false,
  cameraSticky = false,
}) => {
  ...
  const lineColor = useSelector(s => getLineColor(isPublicView, getEditorTheme(s.user)))
  const lineColorDark = useSelector(s => getLineColorDark(isPublicView, getEditorTheme(s.user)))
      ...
      {(ext !== 'spz' && ext !== 'hcap') && (src || model)
        ...
        : (
          <>
            ...
            {(ext === 'hcap') && isSourceUrl && <Hcap src={src} onLoad={onHcapInfo} />}
          </>
        )}
