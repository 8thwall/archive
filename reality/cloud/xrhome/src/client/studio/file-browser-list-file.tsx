...
import {extractBackendFilename} from '../editor/backend-config/backend-config-files'
...
import {downloadEditorFile} from '../common/download-utils'
...
const FileBrowserListFile: React.FC<IFileBrowserListFile> = (
  ...
  const content = useFileContent(fileLocation)
  const {status, dirty} = useChangesetStatus(extractedFilePath)
  ...
  const isBackend = isBackendPath(extractedFilePath)

  const fileBasename = basename(extractedFilePath)

  const name = isBackend ? extractBackendFilename(extractedFilePath) : fileBasename

  const assetContent = isAssetPath(extractedFilePath) && content
  ...
  const handleRenameSubmit = () => {
    ...
    onRename(newFileName, fileLocation)
  }
  ...
  const options = [
    allowRevertDiff && {
      ...
      onClick: () => onRevert(fileLocation),
    },
    allowRevertDiff && {
      content: t('file_list.option.view_diff'),
      onClick: () => onDiff(fileLocation),
    },
    allowDeleteRename && {
    ...
    !isBackend && {
      content: t('file_list.option.download'),
      onClick: () => downloadEditorFile(extractedFilePath, content, name),
    },
