...
const StudioDebugControlsTray: React.FC<IDebugControlsTray> = ({onPlay, simulatorId}) => {
  ...
  const needsSave = useGitsNeedsSave({ignoreExpanse: true}) && Build8.PLATFORM_TARGET !== 'desktop'
  const {notifySimulatorBuildRequired} = useActions(gitActions)
  const fileActionsContext = useFileActionsContext()

  const handlePlay = () => {
    if (needsSave) {
      notifySimulatorBuildRequired(app.appKey)
    }
    onPlay()
    if (needsSave) {
      fileActionsContext.build()
    }
  }
