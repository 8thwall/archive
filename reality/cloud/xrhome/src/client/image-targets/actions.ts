...
const fetchAllImageTargetsForApp = (
  appUuid: string
) => async (dispatch): Promise<IImageTarget[]> => {
  const imageTargets: IImageTarget[] = []
  let continuation = ''
  ...
  do {
    ...
    const res = await dispatch(
      authenticatedFetch(`/v1/image-targets/${appUuid}?${params.toString()}`)
    )

    imageTargets.push(...res.imageTargets.map(derivedImageTarget))
