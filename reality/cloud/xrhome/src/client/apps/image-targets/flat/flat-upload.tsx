...
import uploadFlatBox from '../../../static/uploadFlatBox.svg'
import uploadFlatLabel from '../../../static/uploadFlatLabel.svg'
...
interface IFlatUpload extends ConicalWizardStep {
  image: string
  error: string
  setFile(file: File): void
}

const FlatUpload: React.FunctionComponent<IFlatUpload> = ({
    ...
    <>
      <div className='image-target-editor'>
        <div className='main control'>
          <h3>{t('image_target_page.flat_image_modal.upload_image.heading')}</h3>
          <p>{t('image_target_page.flat_image_modal.upload_image.instruction')}</p>
          {!!error && <Message error>{error}</Message>}
          <div className={classes.instructionImages}>
            <img alt='a box' src={uploadFlatBox} />
            <img alt='an flat label of the box' src={uploadFlatLabel} />
          </div>
          ...
          {image
            ...
            : (
              <UploadDrop
                ...
                dropMessage={t('image_target_page.flat_image_modal.upload_image.drag_drop')}
                onDrop={onDrop}
                fileAccept='.jpg,.jpeg,.png'
                noButton
                className={classes.uploadDrop}
              >
                <Icon stroke='image' size={2} color='gray4' />
                <h3>{t('upload.drag_and_drop', {ns: 'common'})}</h3>
                <p>{t('image_target_page.flat_image_modal.upload_image.drag_drop')}</p>
              </UploadDrop>
            )}
        </div>
      </div>
