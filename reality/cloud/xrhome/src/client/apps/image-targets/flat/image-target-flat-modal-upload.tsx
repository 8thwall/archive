...
import FlatUpload from './flat-upload'
import FlatSetTrackingRegion from './flat-set-tracking-region'
import {
  loadImage,
  getLuminosity, getImageDataRotated, getImageBlobFromUrl, getImageDataWithBackground,
  getDownScaledImage, isUsableDimensions,
} from '../image-helpers'
import {
...
interface IImageTargetFlatModalUpload {
  modalOpen: boolean
  onClose(): void
  onUploadComplete(it: IImageTarget): void
}

const ImageTargetFlatModalUpload: React.FC<IImageTargetFlatModalUpload> = ({
  ...
  const processFile = (name, data) => {
    setName(makeFileName(name, otherImageNames))
    ...
    loadImage(data, imgPool)
      ...
      .then((img) => {
        ...
        setRotatedImage(getImageDataWithBackground(
          img, fillColor, true, fileType, canvasPool
        ).dataURL)
  ...
  const uploadFlatImageTarget = async () => {
    setProcessing(true)
    ...
    const it = await uploadImageTarget(appUuid, imageBlob, name, 'PLANAR', cropPixels, {
      isRotated,
  ...
  const STEP_TO_WIDGET = [
    <FlatUpload
      ...
      error={error}
    />,
    <FlatSetTrackingRegion
      onNext={uploadFlatImageTarget}
  ...
  return (
    ...
    >
      <h2>{t('image_target_page.flat_image_modal.heading')}</h2>
      {stepWidget}
    </Modal>
  )
}

export default ImageTargetFlatModalUpload
