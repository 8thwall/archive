...
class CMAdapter {
  ...
  /**
   * This property is typed as the return type of the Vim function in keymap_vim.js
   * @type {ReturnType<typeof import('./cm/keymap_vim').Vim>}
   */
  static Vim  = null
  ...
  handleKeyDown = (e) => {
    ...
    if (CMAdapter.keyMap[keymap] && CMAdapter.keyMap[keymap].call) {
      ...
      if (cmd) {
        e.preventDefault()
        e.stopPropagation()
        // NOTE(johnny): cmd can be the string 'goCharLeft' by pressing Backspace in replace mode.
        // This is done to prevent the default action of backspace but not actually call a function.
  ...
  getSelection(exclusive = false) {
    const list = []
    const {editor} = this
    editor.getSelections().map((sel) => {
      list.push(editor.getModel().getValueInRange(
        exclusive ? {...sel, endColumn: sel.endColumn + 1} : sel
      ))
    })
    return list.join('\n')
  }
  ...
  listSelections() {
    ...
    return selections.map((sel) => {
      ...
      const head = this.clipPos(toCmPos(this.getHeadForSelection(sel)))

      return {
        anchor: this.clipPos(toCmPos(this.getAnchorForSelection(sel))),
        // NOTE(johnny): Offset ch by +1 because of this issue in monaco-vim.
        // https://github.com/brijeshb42/monaco-vim/issues/89
        head: {...head, ch: head.ch + 1},
      }
    })
  }
