...
import {lintAce, lintFix, lintMonaco} from './linter'
import {parseComponentAst} from '../../../shared/ecs/shared/parse-component-ast'

// Ace doesn't know about or send filenames to the worker, but the linter needs to know the file
// kind. Ace does allow for specifying a classname to be passed when initializing workers.
// Internally ace uses this to initialize the worker by reflection, but this particular worker is
// not directly managed by Ace. Instead, we repurpose these class names to map to pseudo-filenames
// that eslint can use to set the proper configuration.
const CLASS_TO_FILENAME = {
  JavaScriptWorker: 'file.js',
  JsxWorker: 'file.jsx',
  TypeScriptWorker: 'file.ts',
  TsxWorker: 'file.tsx',
}

let filenameForLinting_ = ''
...
// The editor line array is an array of each line, but after the last line we need to add a newline
// to conform to POSIX.
const getLines = doc => `${doc.lines.join('\n')}\n`
...
const fix = (val, id, requestId) => {
  const msg = lintFix(val, id)
  // @ts-ignore
  postMessage({
    fixed: msg.fixed,
    val: msg.output,
...
const process = () => {
  const text = getLines(doc_)
  const errors = lintAce(text, filenameForLinting_)
  postMessage({
    type: 'event',
    name: 'annotate',
    data: errors,
...
const delayProcess = debounce(process, 500)

onmessage = (args) => {
  const {requestId, command, args: arg, data, event, val, id, init, classname, fileName} = args.data
  if (init) {
    filenameForLinting_ = CLASS_TO_FILENAME[classname] || ''
  }
  if (command) {
    if (command === 'lintfixandformat') {
      fix(val, id, requestId)
      return
    }
    if (command === 'setValue') {
      setValue(doc_, arg[0])
    }
    if (command === 'lint-monaco') {
      const errors = lintMonaco(val, fileName)
      postMessage({
        markers: errors,
        requestId,
      })
