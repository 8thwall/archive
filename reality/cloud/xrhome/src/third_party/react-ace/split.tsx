...
import {Annotation, Editor, Selection, UndoManager, Range} from 'ace-builds/src-min-noconflict/ace'
...
export interface ISplitEditorProps {
  ...
  // TODO(pawel) if we ever want split view other than 2 panes we'll need to generalize lineNumbers
  leftLineNumbers?: (string | number)[]
  rightLineNumbers?: (string | number)[]
...
> {
  ...
  public static defaultProps: Partial<ISplitEditorProps> = {
    ...
    leftLineNumbers: null,
    rightLineNumbers: null,
  };
  ...
  public componentDidMount() {
    ...
    split.forEach((editor: IAceEditorClass, index: number) => {
      ...
      editor.renderer.on('afterRender', this.refreshLineNumbers)
      editor
        .getSession()
      ...
      editor.session.on('changeScrollLeft', this.onScroll)
      editor.setValue(
      ...
      const newGutterDecorations = get(gutterDecorations, index, [])
      if (newGutterDecorations && newGutterDecorations.length > 0) {
        this.handleGutterDecorations(editor.getSession(), newGutterDecorations)
      }
  ...
  public componentDidUpdate(prevProps: ISplitEditorProps) {
    ...
    split.forEach((editor: IAceEditorClass, index: number) => {
      ...
      const newGutterDecorations = get(nextProps.gutterDecorations, index, []) as IGutterDecoration[]
      const oldGutterDecorations = get(prevProps.gutterDecorations, index, []) as IGutterDecoration[]

      if (!isEqual(newGutterDecorations, oldGutterDecorations) && Array.isArray(newGutterDecorations)) {
        this.handleGutterDecorations(editor.getSession(), newGutterDecorations, oldGutterDecorations)
      }
    })
    ...
    if (!isEqual(this.props.leftLineNumbers, prevProps.leftLineNumbers) ||
      !isEqual(this.props.rightLineNumbers, prevProps.rightLineNumbers)) {
  ...
  // NOTE(pawel) this assumes that there are two splits
  public refreshLineNumbers = () => {
    const editors = [...this.refEditor.children].filter(c => [...c.classList].includes('ace_editor'))

    const leftGutter = [...editors[0].children].find(c => [...c.classList].includes('ace_gutter')).children[0].children
    const rightGutter = [...editors[1].children].find(c => [...c.classList].includes('ace_gutter')).children[0].children

    const {leftLineNumbers, rightLineNumbers} = this.props

    const [leftEditor, rightEditor] = this.editor.env.split.$editors
    const startLeft = leftEditor.renderer.getFirstVisibleRow()
    const startRight = rightEditor.renderer.getFirstVisibleRow()

    for (let i = 0; i < leftGutter.length; i += 1) {
      if (leftLineNumbers && leftLineNumbers.length > 0) {
        leftGutter[i].firstChild.textContent = `${(leftLineNumbers[startLeft + i]) || ''}`
      } else {
        leftGutter[i].firstChild.textContent = `${startLeft + i + 1}`
      }
    }

    for (let i = 0; i < rightGutter.length; i += 1) {
      if (rightLineNumbers && rightLineNumbers.length > 0) {
        rightGutter[i].firstChild.textContent = `${(rightLineNumbers[startRight + i]) || ''}`
      } else {
        rightGutter[i].firstChild.textContent = `${startRight + i + 1}`
      }
    }
  }

  public handleGutterDecorations = (session, gutterDecorations: IGutterDecoration[], prevGutterDecorations?: IGutterDecoration[]) => {
    ...
    if (gutterDecorations) {
      gutterDecorations.forEach((decoration) => {
