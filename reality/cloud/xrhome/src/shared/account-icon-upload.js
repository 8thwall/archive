...
const transformForResolution = (IconId, maxSize) => makeSharpTransform(
  `s3IconImage-${maxSize}`,
  `web/accounts/icons/${IconId}-${maxSize}x${maxSize}`,
  req => croppedSharp(req).resize(null, maxSize, {withoutEnlargement: true})
)

/**
 * Uploads the given account icon to S3, returning the unique icon ID used when storing the icon.
 */
const uploadIcon = (req, res) => new Promise(
  (resolve, reject) => {
    const iconId = generateIconId()
    const fileFilter = (_, file, cb) => {
      if (!STANDARD_IMAGE_TYPES.some(type => file.mimetype === type)) {
        return cb(new ImageTypeError('File must be a PNG or JPEG', file.mimetype), false)
      }
      return cb(null, true)
    }
    return multer({
