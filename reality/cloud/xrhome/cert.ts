...
export function getCert(key = null, cert = null, name = 'xrhome-local-server') {
  const options = {
    requestCert: false,
    key,
    cert,
  ...
  if (!options.key || !options.cert) {
    ...
    if (certExists) {
      ...
      if (certAge > days) {
        console.log(`SSL Certificate is more than ${days} days old. Regenerating...`)
    ...
    if (!certExists) {
      console.log(`Generating SSL Certificate ${certPath}`)
