...
import consoleConfig from './webpack.console'
import appsConfig from './webpack.apps'
...
import {wrapBazel, bazelBuild} from './tools/gulp/bazel'
import {createLiveBuildIfReplacements} from './tools/gulp/buildif-watch'

const OUTPUT_DIR = 'dist'
const BUILD_INFO_OUTPUT = 'deployment/build-info'
...
fs.writeFileSync(`${BUILD_INFO_OUTPUT}/versionId`, deploymentHash)
const deploymentPath = `static/deployment/${deploymentHash}`

const outputDeploymentPath = path.join(OUTPUT_DIR, deploymentPath)
...
const config = async () => {
  ...
  const workerConfigForMain = workerConfig({
    ...webpackEnv,
    extraPlugins: workerExtraPlugins,
  })
  workerBundle = webpack(workerConfigForMain)
  mainBundle = webpack(mainConfig({...webpackEnv, extraPlugins: mainExtraPlugins}))
  ...
  consoleBundle = webpack(consoleConfig({...webpackEnv}))
  appsBundle = webpack(appsConfig({...webpackEnv}))
}

const copyWasm = () => {
  const wasm = './src/client/static/g8-git/g8-git-asm.wasm'
  const wasmMap = './src/client/static/g8-git/g8-git-asm.wasm.map'
  const wasmFiles = [wasm]
  if (!isProduction) {
    // Append source map in debug builds.
    wasmFiles.push(wasmMap)
  }

  return src(wasmFiles).pipe(dest(`${outputDeploymentPath}/client`))
}

const copyWasmDesktop = () => {
  const wasm = './src/client/static/g8-git/g8-git-asm.wasm'
  const wasmMap = './src/client/static/g8-git/g8-git-asm.wasm.map'
  const wasmFiles = [wasm]
  if (!isProduction) {
  ...
  return src(wasmFiles).pipe(dest(`${DESKTOP_OUTPUT_DIR}/worker/client`))
}

const symlinkSrc = async () => {
  if (!isProduction) {
    // Create symlinks to the source.
    await new Promise<void>((resolve, reject) => {
      fs.symlink(`${CODE8}/c8`, `./${outputDeploymentPath}/client/c8`, (err) => {
        if (err) {
          return reject(err)
        }
        return resolve()
    ...
    return new Promise<void>((resolve, reject) => {
      const target = `${CODE8}/bazel-code8/external`
      const relativePath = `./${outputDeploymentPath}/client/external`
      fs.symlink(target, relativePath, (err) => {
        if (err) {
          return reject(err)
        }
        return resolve()
...
const cleanProtos = () => del([
  './src/shared/integration/gar/proto',
  './src/shared/integration/vps/proto',
])
const buildProtos = () => bazelBuild('//reality/cloud/xrhome:protos')

const copyVpsProtos = () => (
  src(`${CODE8}/bazel-bin/reality/cloud/vps/proto/**.ts`)
    .pipe(dest('./src/shared/integration/vps/proto'))
)

const syncProtos = series(cleanProtos, buildProtos, copyVpsProtos)

const copySemantic = () => (
  src('./semantic/dist/**').pipe(dest('./src/client/static/semantic/dist'))
)

const triggerSemanticIfNotExists = async () => {
  if (!fs.existsSync('./semantic/dist/semantic.min.js')) {
    await buildSemantic()
  }
}

const ensureSemantic = series(
  triggerSemanticIfNotExists,
  copySemantic
)

const copyPublic = () => (
  src('src/client/static/public/**').pipe(dest(`${OUTPUT_DIR}/static/public/`))
)

const copyRoot = () => (
  src('src/client/static/root/**').pipe(dest(`${OUTPUT_DIR}/`))
)

const buildG8 = () => {
  const buildFlags = isProduction
    ? ['-c', 'opt', '--copt', '-DC8_RELEASE_CONFIG_ENABLE_CONSOLE_LOGGING=false']
    : ['--verbose_failures', '-c', 'dbg']

  if (process.env.DEBUG_G8) {
    buildFlags.push('--copt', `'--source-map-base /${deploymentPath}/client/'`)
  }

  buildFlags.push('--config=wasm')

  return bazelBuild(['//c8/git:g8-git-asm', '//c8/git:g8-api.capnp-js'], buildFlags)
}

const copyG8 = () => {
  const cap = `${CODE8}/bazel-bin/c8/git/g8-api.capnp.js`
  const asm = `${CODE8}/bazel-bin/c8/git/g8-git-asm.js`
  const wasm = `${CODE8}/bazel-bin/c8/git/g8-git-asm.wasm`
  const wasmMap = `${CODE8}/bazel-bin/c8/git/g8-git-asm.wasm.map`
  const buildOutputs = [cap, asm, wasm]
  if (!isProduction) {
    // Append source map in debug builds.
    buildOutputs.push(wasmMap)
  }

  const outputDir = './src/client/static/g8-git/'
  return src(buildOutputs).pipe(dest(outputDir))
}
...
const buildMain = () => runBundle(mainBundle)
const buildWorker = () => runBundle(workerBundle)
const buildConsole = () => runBundle(consoleBundle)
const buildApps = () => runBundle(appsBundle)
...
const prepareCert = async () => {
  getCert()
}

const hotReloadMain = () => hotReloadBundle(mainBundle, 3601)
const watchMain = () => watchBundle(mainBundle)
const watchWorker = () => watchBundle(workerBundle)
const watchConsole = () => watchBundle(consoleBundle)
const watchApps = () => watchBundle(appsBundle)
...
const defaultTask = series(
  ...
  parallel(
    watchApps,
    watchConsole,
    process.env.HOT === '1' ? hotReloadMain : watchMain,
    watchWorker
  )
)

const cleanDesktop = () => del([
  DESKTOP_OUTPUT_DIR,
])

const copyDesktopPublic = () => (
  src('src/client/static/public/**').pipe(dest(`${DESKTOP_OUTPUT_DIR}/static/public/`))
)
...
const bundleAnalyzeMain = series(prodEnv, injectBundleAnalyzer(mainExtraPlugins), defaultTask)
const bundleAnalyzeWorker = series(prodEnv, injectBundleAnalyzer(workerExtraPlugins), defaultTask)
