...
:::info
La función backend debe exportar un **método async** llamado `handler`.  Por favor, consulte la documentación [Writing Backend Code](/studio/essentials/modules/backend-services/backend-functions/writing-backend-code/) para más detalles.
:::
...
const handler = async (event: any) => {
  // El código personalizado del backend va aquí
