...
| cuerpo                | cadena                                            | Llama a `JSON.parse(event.body)` para transformar el cuerpo en un objeto.                                                                         |
...
| cuerpo     | cadena                                            | El objeto body `JSON.stringify()` asociado a la respuesta.                 |
...
```
{
  "error": {
    "name": "TypeError",
    "message": "No se pueden leer propiedades de undefined (leyendo 'foo')",
