...
`onProcessCpu()` se llama para leer los resultados del procesamiento de la GPU y devolver los datos utilizables. Llamada con `{ frameStartResult, processGpuResult }`. Los datos devueltos por los módulos en [`onProcessGpu`](onprocessgpu.md) estarán presentes como `processGpu.modulename` donde el nombre viene dado por module.name = "modulename".
...
Cualquier dato que desees proporcionar a [`onUpdate`](onupdate.md) debe ser devuelto. Será proporcionado a ese método como `processCpuResult.modulename`
