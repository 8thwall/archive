...
Establece un proveedor que pasa una lista de texturas de primer plano para dibujar. Debe ser una función que tome las mismas entradas que [`cameraPipelineModule.onUpdate`](/api/camerapipelinemodule/onupdate).
...
Las texturas de primer plano se dibujarán sobre la textura proporcionada llamando a [`XR8.GlTextureRenderer.setTextureProvider()`](filltextureviewport.md). Las texturas del primer plano se dibujarán en el orden de la matriz devuelta.
