...
このイベントは[`xrhand`](/legacy/api/aframe/#hand-tracking)によって、その後ハンドが見つかったときに発行される。

`xrhandupdated.detail : {id、transform、頂点、法線、handKind、attachmentPoints}`。
...
| attachmentPoints：{name, position： {x,y,z} }                                          | アタッチメントポイントの名前。 アタッチメントポイントの名前。 使用可能なアタッチメントポイントのリストについては、[`XR8.HandController.AttachmentPoints`](/legacy/api/handcontroller/attachmentpoints) を参照してください。 position\\`はトランスフォームからの相対位置である。 |
...
| プロパティ                            | 説明                                                                                                                                                        |
| -------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 名称                               | アタッチメントポイントの名前。 アタッチメントポイントの名前。 使用可能なアタッチメントポイントのリストについては、[`XR8.HandController.AttachmentPoints`](/legacy/api/handcontroller/attachmentpoints) を参照してください。 |
