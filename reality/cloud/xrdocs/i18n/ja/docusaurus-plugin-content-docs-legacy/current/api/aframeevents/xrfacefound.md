...
このイベントは[`xrface`](/legacy/api/aframe/#face-effects)によって、顔が最初に見つかったときに発行される。

`xrfacefound.detail : {id、transform、頂点、法線、attachmentPoints}`。
...
| attachmentPoints：名前、位置： {x,y,z} }\\`                                                | 使用可能なアタッチメントポイントのリストについては、[`XR8.FaceController.AttachmentPoints`](/legacy/api/facecontroller/attachmentpoints) を参照してください。 position\`はトランスフォームからの相対位置である。 position`はトランスフォームからの相対位置である。 |
