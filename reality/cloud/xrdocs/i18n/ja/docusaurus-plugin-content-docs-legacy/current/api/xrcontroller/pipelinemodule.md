...
| ワールドポイント    | id、confidence、position: {x, y, z}}]`[{id、confidence、position: }]`。 | シーン内の位置で検出されたワールド内の点の配列。 シーン内の位置で検出されたワールド内の点の配列。 XrController`がワールドポイントを返すように設定されていて、`trackingReason != 'INITIALIZING'\\`の場合のみ満たされる。 |
