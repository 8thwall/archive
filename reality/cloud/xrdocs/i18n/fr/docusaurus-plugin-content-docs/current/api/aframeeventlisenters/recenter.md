...
Si l'origine et l'orientation ne sont pas fournies, la caméra est réinitialisée à l'origine spécifiée précédemment par un appel à `recentrage` ou par le dernier appel à `XR8.XrController.updateCameraProjectionMatrix()` lors de l'utilisation de `xrweb` ou `XR8.FaceController.configure({coordinates: {origin, scale, axes}})` / `XR8.LayersController.configure({coordinates: {origin, scale, axes}})` lors de l'utilisation de `xrface` ou `xrlayers`.

**IMPORTANT :** Avec le A-Frame, `updateCameraProjectionMatrix()` et/ou `configure()` est initialement appelé en fonction de la position initiale de la caméra dans la scène.
...
| de face : {w, x, y, z} [Optionnel] | Un quaternion représentant la direction de la caméra à l'origine. |
