...
## Impossible de demander l'activation d'un VPS {#unable-to-request-a-vps-activation}

#### Numéro {#issue}

Lors de l'activation d'un emplacement VPS, une alerte indiquant "ERROR : Unable to request a Vps Activation"\` apparaît.

#### Résolution {#resolution}

Aucune action n'est requise. Cette erreur indique que la limite quotidienne d'activation globale a été atteinte. Le lieu est ajouté à une file d'attente et activé dès que possible, généralement dans les 24 heures.
...
## Test Scan n'apparaît pas dans le navigateur géospatial {#test-scan-not-showing-up-in-the-geospatial-browser}
