...
:::info
La fonction backend doit exporter une méthode **async** appelée `handler`.  Veuillez vous référer à la documentation [Writing Backend Code](/studio/guides/modules/backend-services/backend-functions/writing-backend-code/) pour plus de détails.
:::
