...
Certaines matrices, telles que les matrices TRS, ont des inverses efficacement calculables. Dans ces cas, Mat4 permet de calculer l'inverse en temps constant, ce qui en fait une opération O(1). Les objets Mat4 sont créés à l'aide de la fabrique ecs.math.mat4 (Mat4Factory) ou par des opérations sur des instances Mat4 existantes.
...
Crée une matrice en utilisant les données de ligne spécifiées. Vous pouvez également fournir, à titre facultatif, des données sur la ligne inverse. Les dataRows et les inverseDataRows doivent être des tableaux, chacun contenant quatre nombres. Si l'inverse n'est pas fourni, il sera calculé automatiquement si la matrice est inversible.
...
Inverser la matrice ou jeter si la matrice n'est pas inversible. Comme Mat4 stocke l'inverse précalculé, cette opération est O(1).
...
Inverser la matrice ou jeter si la matrice n'est pas inversible. Comme Mat4 stocke l'inverse précalculé, cette opération est O(1). Stocke le résultat dans ce Mat4 et renvoie ce Mat4 pour le chaînage.
...
Fixe la matrice au résultat de cette matrice multiplié par m. Stocke le résultat dans ce Mat4 et renvoie ce Mat4 pour le chaînage.
