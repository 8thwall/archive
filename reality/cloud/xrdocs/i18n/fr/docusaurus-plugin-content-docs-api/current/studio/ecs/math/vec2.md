...
Calculer le produit croisé de ce vecteur et d'un autre vecteur. Pour les vecteurs 2D, le produit croisé est la taille de la composante z du produit croisé 3D des deux vecteurs avec 0 comme composante z.

```ts
existingVec2.cross(v : Vec2Source) // -> nombre
```
...
```ts
existingVec2.distanceTo(v : Vec2Source) // -> nombre
```
...
```ts
existingVec2.dot(v : Vec2Source) // -> nombre
```
