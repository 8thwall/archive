...
Créer un Quat représentant la rotation nécessaire pour qu'un objet positionné à "œil" regarde un objet positionné à "cible", avec le vecteur "haut" donné.
...
```ts
existingQuat.dot(target : QuatSource) // -> nombre
```
...
```ts
existingQuat.radiansTo(target : QuatSource) // -> nombre
```
...
Définir ce quaternion comme le résultat de q fois ce quaternion. Stocke le résultat dans ce Quat et renvoie ce Quat pour le chaînage.
...
Réglez le quaternion sur une rotation qui amènerait l'œil à regarder la cible avec le vecteur donné. Stocke le résultat dans ce Quat et renvoie ce Quat pour le chaînage.
