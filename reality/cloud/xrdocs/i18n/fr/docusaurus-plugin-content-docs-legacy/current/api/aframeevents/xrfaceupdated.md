...
Cet événement est émis par [`xrface`](/legacy/api/aframe/#face-effects) lorsque le visage est trouvé par la suite.
...
| attachmentPoints : { name, position : {x,y,z} }                                                                | Voir [`XR8.FaceController.AttachmentPoints`](/legacy/api/facecontroller/attachmentpoints) pour la liste des points d'attache disponibles. `position` est relative à la transformation. |
