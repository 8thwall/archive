...
**chargement manuel** : Se déclenche au début du chargement pour les ressources supplémentaires de l'AR manuel.
...
**balayage manuel** : Se déclenche lorsque toutes les ressources AR ont été chargées et que le balayage a commencé.
...
**handfound** : Se déclenche lorsqu'une main est trouvée pour la première fois.
...
| points d'attachement | `{nom, position : {x,y,z}}`   | Voir [`XR8.HandController.AttachmentPoints`](attachmentpoints.md) pour la liste des points d'attache disponibles. `position` est relative à la transformation.                                   |
...
**handlost** : Se déclenche lorsqu'une main n'est plus suivie.
