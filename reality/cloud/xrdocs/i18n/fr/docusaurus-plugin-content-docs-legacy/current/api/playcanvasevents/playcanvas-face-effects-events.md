...
Les événements Face Effects peuvent être écoutés en tant que `this.app.on(event, handler, this)`.
...
**xr:facescanning** : Se déclenche lorsque toutes les ressources AR des visages ont été chargées et que l'analyse a commencé.
...
```javascript
  ...
  // Se déclenche lorsque le chargement commence pour les ressources AR de face supplémentaires.
