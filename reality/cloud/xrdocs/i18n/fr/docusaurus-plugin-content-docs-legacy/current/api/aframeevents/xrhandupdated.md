...
Cet événement est émis par [`xrhand`](/legacy/api/aframe/#hand-tracking) lorsque la main est trouvée par la suite.
...
| attachmentPoints : { name, position : {x,y,z} }            | Voir [`XR8.HandController.AttachmentPoints`](/legacy/api/handcontroller/attachmentpoints) pour la liste des points d'attache disponibles. Le nom du point d'attache.                             |
