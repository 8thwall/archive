...
**onXrImageScanningObservable** : Se déclenche lorsque toutes les images de détection ont été chargées et que la numérisation a commencé.

`onXrImageScanningObservable : { imageTargets : {nom, type, métadonnées, géométrie} }`

**onXrImageFoundObservable** : Se déclenche lorsqu'une cible d'image est trouvée pour la première fois.
...
**onXrImageUpdatedObservable** : Se déclenche lorsqu'une cible d'image change de position, de rotation ou d'échelle.
...
**onXrImageLostObservable** : Se déclenche lorsqu'une cible d'image n'est plus suivie.
...
**onFaceScanningObservable** : Se déclenche lorsque toutes les ressources AR pour les visages ont été chargées et que le balayage a commencé.
...
**onRightEyeOpenedObservable** : Se déclenche lorsque l'œil droit d'un visage suivi s'ouvre
...
**onBlinkedObservable** : Se déclenche lorsque les yeux d'un visage suivi clignotent.
...
#### Image Cible Exemple {#image-target-example}
...
```javascript
// Ceci est appelé lorsque le visage est trouvé pour la première fois.  Elle fournit des informations statiques sur le
// visage, telles que les UV et les indices
...
// ceci est appelé à chaque fois que le visage est mis à jour, c'est-à-dire à chaque image
