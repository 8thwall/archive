...
et que la caméra démarre. C'est la première chose que voient vos utilisateurs lorsqu'ils entrent sur le site
votre expérience WebAR.

Cette section décrit comment personnaliser l'écran de chargement en fournissant des valeurs qui modifient la couleur de
, la barre d'outils de chargement et l'animation de chargement afin de les adapter au design général de votre expérience.
...
## ID's / Classes à remplacer par {#ids--classes-to-override}
...
| loadImage                           | ID                 | L'ID d'une image.  L'image doit être un `<a-asset>` (voir ci-dessus). Écran de chargement n° 4)                                                       |
...
observer.observer(document.body, {childList: true})
```
