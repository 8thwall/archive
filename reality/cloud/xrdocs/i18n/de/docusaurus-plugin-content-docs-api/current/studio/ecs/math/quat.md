...
z: Zahl\`\` Zugriff auf die z-Komponente des Quaternions.
...
Erstellen Sie einen Quat aus einer Achsen-Winkel-Darstellung. Die Richtung des aa-Vektors gibt die Drehachse an, und der Betrag des Vektors gibt den Winkel in Radiant an. Zum Beispiel stellt quat.axisAngle(vec3.up().scale(Math.PI / 2)) eine 90-Grad-Drehung um die y-Achse dar und ist äquivalent zu quat.yDegrees(90). Wenn ein Ziel angegeben wird, wird das Ergebnis im Ziel gespeichert und das Ziel zurückgegeben. Andernfalls wird ein neuer Quat erstellt und zurückgegeben.
...
Erstellen Sie eine Quat, die die Drehung darstellt, die erforderlich ist, damit ein am "Auge" positioniertes Objekt ein am "Ziel" positioniertes Objekt mit dem angegebenen "Aufwärtsvektor" betrachten kann.
...
```ts
existingQuat.dot(target: QuatSource) // -> Zahl
```
...
Ermittelt die normalisierte Version dieser Quaternion mit der Länge 1.
...
```ts
existingQuat.radiansTo(target: QuatSource) // -> Zahl
```
...
Setzen Sie diese Quaternion als das Ergebnis von q mal dieser Quaternion. Speichern Sie das Ergebnis in diesem Quat und geben Sie diesen Quat zur Verkettung zurück.
...
Setzen Sie die Quaternion auf eine Drehung, die dazu führt, dass das Auge das Ziel mit dem angegebenen Vektor anschaut. Speichern Sie das Ergebnis in diesem Quat und geben Sie diesen Quat zur Verkettung zurück.
