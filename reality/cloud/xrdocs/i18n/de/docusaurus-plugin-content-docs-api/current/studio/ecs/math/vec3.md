...
`readonly y: number` Zugriff auf die y-Komponente des Vektors.

`readonly z: number` Zugriff auf die z-Komponente des Vektors.
...
Berechne das Kreuzprodukt aus diesem Vektor und einem anderen Vektor.
...
Zugriff auf den Vektor als homogenes Array (vier Dimensionen).
...
```ts
existingVec3.distanceTo(v: Vec3Source) // -> Zahl
```
...
```ts
existingVec3.dot(v: Vec3Source) // -> Zahl
```
...
Berechne eine lineare Interpolation zwischen diesem Vektor und einem anderen Vektor v mit einem Faktor t, so dass das Ergebnis thisVec \* (1 - t) + v \* t ist. Der Faktor t sollte zwischen Null und 1 liegen.
...
Die folgenden Methoden führen Berechnungen auf der Grundlage des aktuellen Wertes eines Vec3 durch und ändern dessen Inhalt an Ort und Stelle. Sie sind parallel zu den Methoden in der obigen veränderlichen API. Methoden, die Vec3-Typen zurückgeben, geben einen Verweis auf das aktuelle Objekt zurück, um die Methodenverkettung zu erleichtern. Veränderbare APIs können leistungsfähiger sein als unveränderliche APIs, sind aber in der Regel weniger sicher, weniger lesbar und fehleranfälliger.

### SetCross

Berechne das Kreuzprodukt aus diesem Vektor und einem anderen Vektor. Speichert das Ergebnis in diesem Vec3 und gibt diesen Vec3 zur Verkettung zurück.
...
```ts
existingVec3.setMix(v: Vec3Source, t: Zahl) // -> vec3
```

### setNormalize

Setzt den Vektor auf eine Version seiner selbst mit der gleichen Richtung, aber mit der Länge 1. Speichert das Ergebnis in diesem Vec3 und gibt diesen Vec3 zur Verkettung zurück.
...
```ts
existingVec3.setScale(s: Zahl) // -> vec3
```
...
Legen Sie diesen Vec3 so fest, dass er denselben Wert wie ein anderer Vec3 oder ein anderes Objekt mit den Eigenschaften x, y und z hat. Speichert das Ergebnis in diesem Vec3 und gibt diesen Vec3 zur Verkettung zurück.
...
```ts
existingVec3.setXyz(x: Zahl, y: Zahl, z: Zahl) // -> vec3
```
