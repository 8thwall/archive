...
## iframe Setup für Android und iOS 15+ {#iframe-setup-for-android-and-ios-15}

Um Inline-AR für Android und iOS 15+ zuzulassen, müssen Sie einen allow-Parameter in Ihren iframe mit
die folgenden [feature-policy directives](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Permissions-Policy#directives) einfügen:
...
Das Folgende ist **NUR** für die Unterstützung von Inline AR in iOS-Versionen vor iOS 15 erforderlich. In Anbetracht
der hohen Akzeptanz von iOS 15+ empfehlen wir diesen Ansatz **NICHT MEHR**.
...
Zusätzlich zum Einfügen des allow-Parameters mit den korrekten
[feature-policy directives](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Permissions-Policy#directives)
in Ihren iframe wie oben erklärt, müssen Sie zur Unterstützung von World Tracking-Projekten auf iOS-Versionen vor iOS
15 auch zusätzliches Javascript sowohl auf den OUTER- als auch auf den INNER AR-Seiten einfügen, wie unten
erklärt.

In diesen Versionen blockiert Safari den Zugriff auf Geräteausrichtung und Gerätebewegungsereignisse von herkunftsübergreifenden
iframes. Um dem entgegenzuwirken, müssen Sie zwei Skripte in Ihr Projekt aufnehmen, um die Kompatibilität
mit iOS bei der Bereitstellung von World Tracking-Projekten zu gewährleisten.

Dies ist **nicht erforderlich für Face Effects oder Image Target Projekte** (mit `disableWorldTracking` auf
auf `true` gesetzt).

Bei korrekter Implementierung ermöglicht dieser Prozess der OUTER-Website, Bewegungsereignisse an die
INNER AR-Website zu senden, eine Voraussetzung für World Tracking.
...
##### Innere Seite: Nicht-AFrame-Projekte {#inner-page-non-aframe-projects}
