...
Wenn Ursprung und Ausrichtung nicht angegeben werden, wird die Kamera auf den Ursprung zurückgesetzt, der zuvor durch einen Aufruf von `recenter` oder den letzten Aufruf von `XR8.XrController.updateCameraProjectionMatrix()` bei Verwendung von `xrweb` oder `XR8.FaceController.configure({coordinates: {origin, scale, axes}})` / `XR8.LayersController.configure({coordinates: {origin, scale, axes}})` bei Verwendung von `xrface` oder `xrlayers` angegeben wurde.

**WICHTIG:** Bei A-Frame wird `updateCameraProjectionMatrix()` und / oder `configure()` zunächst auf der Grundlage der anfänglichen Kameraposition in der Szene aufgerufen.

## Parameter {#parameters}

| Parameter                                                                                                | Beschreibung                                                                                         |
| -------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| Herkunft: {x, y, z} [Optional]       | Der Ort des neuen Ursprungs.                                                         |
| Verkleidung: {w, x, y, z} [Optional] | Eine Quaternion, die die Richtung angibt, in die die Kamera am Ursprung zeigen soll. |
