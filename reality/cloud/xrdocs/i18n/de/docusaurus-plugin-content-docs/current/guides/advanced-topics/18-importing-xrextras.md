...
Wenn Sie nur grundlegende Anpassungen des XRExtras-Ladebildschirms vornehmen müssen, lesen Sie bitte stattdessen [diesen Abschnitt](/guides/advanced-topics/load-screen) .
...
5. Für **jede** Datei im Ordner `aframe/components` entfernen Sie die Anweisung `import` und ersetzen sie durch `// @ts-nocheck`

![xrextras deaktiviert die Typüberprüfung](/images/xrextras-disable-type-checking.jpg)

6. Entfernen Sie in head.html den `` Tag für @8thwall.xrextras oder kommentieren Sie ihn aus, damit er nicht mehr von unserem CDN bezogen wird:
...
#### Ändern/Hinzufügen von Bild-Assets {#changingadding-image-assets}
...
Verwenden Sie in **javascript** Dateien einen relativen Pfad und`require()`, um auf Assets zu verweisen:
