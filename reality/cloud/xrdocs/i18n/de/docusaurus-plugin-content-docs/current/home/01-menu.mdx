---
slug: /
displayed_sidebar: null
sidebar_class_name: versteckt
title: 8. Mauer Dokumentation
description: 8. Mauer Dokumentation
---

```mdx-code-block
  <div className="doc-container">

    # Dokumentation
    Erstellen, starten und skalieren Sie erstklassige Webspiele und WebAR-Erlebnisse mit 8th Wall.

    <div class="hero-banner">
      <a href="/docs/migration/" class="hero-link">
        <div class="hero-content">
          <div class="hero-icon">
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
              stroke-width="2"
              stroke-linecap="round"
              stroke-linejoin="round"
              aria-hidden="true"
            >
              <!-- clock -->
              <path d="M12 21a9 9 0 1 0-9-9 9 9 0 0 0 9 9Z"/>
              <path d="M12 7v5l3 2"/>
            </svg>
          </div>

          <div class="hero-text">
            <h2 class="hero-title">Leitfaden zur Migration</h2>
            <p class="hero-description">
              Erfahren Sie mehr über die Änderungen im 8. Wall-Geschäft und wie Sie sich auf Open Source vorbereiten können.
            </p>
          </div>
          <div class="hero-cta">
            Mehr erfahren
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              <path d="M5 12h14"/>
              <path d="m12 5 7 7-7 7"/>
            </svg>
          </div>
        </div>
      </a>
    </div>

    <div className="doc-grid">
      <a href="account" className="doc-card-link">
        <div className="doc-card">
          <div className="doc-card-icon">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              <path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"/>
              <circle cx="12" cy="7" r="4"/>
            </svg>
          </div>
          <div className="doc-card-content">
            <h3 className="doc-card-title">Konto & Abrechnung</h3>
            <p className="doc-card-description">Hier finden Sie die Dokumentation zur Verwaltung Ihres 8th Wall-Kontos.</p>
          </div>
        </div>
      </a>

      <a href="studio" className="doc-card-link">
        <div className="doc-card">
          <div className="doc-card-icon">
            <svg width="24" height="24" viewBox="-1 -6 34 34" fill="currentColor">
              <path fill-rule="evenodd" clip-rule="evenodd" d="M31.9038 16.25C31.9038 14.0867 30.6093 7.18816 30.6093 7.18816L30.6263 7.20519C29.8428 2.41876 27.2537 0 22.859 0H9.06185C4.65016 0 2.07809 2.41876 1.29455 7.20519C1.29455 7.20519 0 14.1038 0 16.267C0 18.4303 1.20938 20.7128 3.88365 20.7128C6.38758 20.7128 7.40959 18.8902 8.10797 17.1698L9.18108 14.6659H22.7227L23.7959 17.1187C24.4942 18.8902 25.5674 20.6958 28.0713 20.6958C30.7456 20.6958 31.9038 18.4133 31.9038 16.25ZM8.89083 7.0548H11.1092C11.5983 7.0548 12 7.45655 12 7.94563C12 8.43471 11.5983 8.83646 11.1092 8.83646H8.89083V11.0548C8.89083 11.5439 8.48908 11.9456 8 11.9456C7.51092 11.9456 7.10917 11.5439 7.10917 11.0548V8.83646H4.89083C4.40175 8.83646 4 8.43471 4 7.94563C4 7.45655 4.40175 7.0548 4.89083 7.0548H7.10917V4.83646C7.10917 4.34738 7.51092 3.94563 8 3.94563C8.48908 3.94563 8.89083 4.34738 8.89083 4.83646V7.0548ZM22.9996 6.44579C22.1509 6.44579 21.4996 5.79448 21.4996 4.9458C21.4996 4.09711 22.1509 3.4458 22.9996 3.4458C23.8483 3.4458 24.4996 4.09711 24.4996 4.9458C24.4996 5.79448 23.8483 6.44579 22.9996 6.44579ZM21.4996 10.9455C21.4996 11.7941 22.1509 12.4455 22.9996 12.4455C23.8483 12.4455 24.4996 11.7941 24.4996 10.9455C24.4996 10.0968 23.8483 9.44547 22.9996 9.44547C22.1509 9.44547 21.4996 10.0968 21.4996 10.9455ZM20 9.44584C19.1513 9.44584 18.5 8.79453 18.5 7.94585C18.5 7.09717 19.1513 6.44586 20 6.44586C20.8487 6.44586 21.5 7.09717 21.5 7.94585C21.5 8.79453 20.8487 9.44584 20 9.44584ZM24.4997 7.94585C24.4997 8.79453 25.151 9.44584 25.9997 9.44584C26.8483 9.44584 27.4997 8.79453 27.4997 7.94585C27.4997 7.09717 26.8483 6.44586 25.9997 6.44586C25.151 6.44586 24.4997 7.09717 24.4997 7.94585Z"></path>
            </svg>
          </div>
          <div className="doc-card-content">
            <h3 className="doc-card-title">Studio</h3>
            <p className="doc-card-description">Hier finden Sie die Dokumentation für 8th Walls neues Studio-Toolkit.</p>
          </div>
        </div>
      </a>

      <a href="api" className="doc-card-link">
        <div className="doc-card">
          <div className="doc-card-icon">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              <polyline points="16 18 22 12 16 6"/>
              <polyline points="8 6 2 12 8 18"/>
            </svg>
          </div>
          <div className="doc-card-content">
            <h3 className="doc-card-title">API</h3>
            <p className="doc-card-description">Zugang zur Dokumentation von 8th Walls JavaScript API.</p>
          </div>
        </div>
      </a>
    </div>

    <div className="resources-section">
      <h2 className="resources-title">Zusätzliche Ressourcen</h2>
      <div className="resources-grid">
        <a href="https://8thwall.com/forum" target="_blank" className="resource-card">
          <div className="resource-icon">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-messages-square-icon lucide-messages-square"><path d="M14 9a2 2 0 0 1-2 2H6l-4 4V4a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2z"/><path d="M18 9h2a2 2 0 0 1 2 2v11l-4-4h-6a2 2 0 0 1-2-2v-1"/></svg>
          </div>
          <div className="resource-content">
            <h3 className="resource-title">Forum</h3>
            <p className="resource-description">Tauschen Sie sich mit anderen Entwicklern aus, stellen Sie Fragen und tauschen Sie sich über Ihre Erfahrungen mit 8th Wall aus.</p>
          </div>
        </a>
        <a href="https://8th.io/discord" target="_blank" className="resource-card">
          <div className="resource-icon">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" class="bi bi-discord" viewBox="0 0 16 16">
              <path d="M13.545 2.907a13.2 13.2 0 0 0-3.257-1.011.05.05 0 0 0-.052.025c-.141.25-.297.577-.406.833a12.2 12.2 0 0 0-3.658 0 8 8 0 0 0-.412-.833.05.05 0 0 0-.052-.025c-1.125.194-2.22.534-3.257 1.011a.04.04 0 0 0-.021.018C.356 6.024-.213 9.047.066 12.032q.003.022.021.037a13.3 13.3 0 0 0 3.995 2.02.05.05 0 0 0 .056-.019q.463-.63.818-1.329a.05.05 0 0 0-.01-.059l-.018-.011a9 9 0 0 1-1.248-.595.05.05 0 0 1-.02-.066l.015-.019q.127-.095.248-.195a.05.05 0 0 1 .051-.007c2.619 1.196 5.454 1.196 8.041 0a.05.05 0 0 1 .053.007q.121.1.248.195a.05.05 0 0 1-.004.085 8 8 0 0 1-1.249.594.05.05 0 0 0-.03.03.05.05 0 0 0 .003.041c.24.465.515.909.817 1.329a.05.05 0 0 0 .056.019 13.2 13.2 0 0 0 4.001-2.02.05.05 0 0 0 .021-.037c.334-3.451-.559-6.449-2.366-9.106a.03.03 0 0 0-.02-.019m-8.198 7.307c-.789 0-1.438-.724-1.438-1.612s.637-1.613 1.438-1.613c.807 0 1.45.73 1.438 1.613 0 .888-.637 1.612-1.438 1.612m5.316 0c-.788 0-1.438-.724-1.438-1.612s.637-1.613 1.438-1.613c.807 0 1.451.73 1.438 1.613 0 .888-.631 1.612-1.438 1.612"/>
            </svg>
          </div>
          <div className="resource-content">
            <h3 className="resource-title">Diskord</h3>
            <p className="resource-description">Treten Sie unserer Entwickler-Community bei und erhalten Sie Echtzeit-Support, Best Practices und Updates zu neuen Funktionen.</p>
          </div>
        </a>
      </div>
    </div>
    <div className="legacy-container">
      <a href="legacy" className="legacy-link">
        <p>Legacy-Dokumentation</p>
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" className="legacy-arrow">
          <line x1="5" y1="12" x2="19" y2="12"/>
          <polyline points="12 5 19 12 12 19"/>
        </svg>
      </a>
    </div>
  </div>
```
