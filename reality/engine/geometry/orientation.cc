...
void makeRightHanded(HandResponse::Builder r) {
  // camera position
  makeRightHanded(r.getCamera().getExtrinsic().getRotation());
  makeRightHanded(r.getCamera().getExtrinsic().getPosition());

  for (auto hand : r.getHands()) {
    // update hand mesh transform
    makeRightHanded(hand.getTransform().getPosition());
    makeRightHanded(hand.getTransform().getRotation());

    auto handVertices = hand.getVertices();
    for (int i = 0; i < HANDMESH_R_UV_VERTEX_COUNT; i++) {
      handVertices.set(i * 3 + 2, -hand.getVertices()[i * 3 + 2]);
    }

    auto handNormals = hand.getNormals();
    for (int i = 0; i < HANDMESH_R_UV_VERTEX_COUNT; i++) {
      handNormals.set(i * 3 + 2, -hand.getNormals()[i * 3 + 2]);
    }

    for (auto attachmentPoint : hand.getAttachmentPoints()) {
      makeRightHanded(attachmentPoint.getPosition());
      makeRightHanded(attachmentPoint.getRotation());
    }

    // update wrist mesh transform
    makeRightHanded(hand.getWristTransform().getPosition());
    makeRightHanded(hand.getWristTransform().getRotation());
  }
}

void makeRightHanded(FrameworkHandResponse::Builder r) {
  auto leftWrist = r.getLeftWristGeometry();
  for (auto localPoint : leftWrist.getVertices()) {
    makeRightHanded(localPoint);
  }

  for (auto normal : leftWrist.getNormals()) {
    makeRightHanded(normal);
  }

  auto rightWrist = r.getRightWristGeometry();
  for (auto localPoint : rightWrist.getVertices()) {
    makeRightHanded(localPoint);
  }

  for (auto normal : rightWrist.getNormals()) {
    makeRightHanded(normal);
  }
}
...
// For hand transformation
void recenterAndScale(CameraCoordinates::Reader system, HandResponse::Builder r) {
  float s = system.getScale() > 0.0f ? system.getScale() : 1.0f;
  auto op = toHVector(system.getOrigin().getPosition());
  auto oq = groundPlaneRotation(toQuaternion(system.getOrigin().getRotation()));
  auto o = cameraMotion(op, oq);
  ...
  // Update face transform and geometry
  for (auto hand : r.getHands()) {
    recenterAndScale(hand.getTransform().getPosition(), o, s);
    recenterAndScale(hand.getTransform().getRotation(), oq);
    hand.getTransform().setScale(hand.getTransform().getScale() * system.getScale());

    // Recentering and scaling the wrist transform has shown to break aframe, so it has been removed
    hand.getWristTransform().setScale(hand.getWristTransform().getScale() * system.getScale());
  }
...
void mirror(HandResponse::Builder r) {
  // mirror the hand transform and geometry
  for (auto hand : r.getHands()) {
    mirror(hand.getTransform().getPosition());
    mirror(hand.getTransform().getRotation());

    auto handVertices = hand.getVertices();
    for (int i = 0; i < HANDMESH_R_UV_VERTEX_COUNT; i++) {
      handVertices.set(i * 3, -hand.getVertices()[i * 3]);
    }
    auto handNormals = hand.getNormals();
    for (int i = 0; i < HANDMESH_R_UV_VERTEX_COUNT; i++) {
      handNormals.set(i * 3, -hand.getNormals()[i * 3]);
    }
    ...
    mirror(hand.getWristTransform().getPosition());
    mirror(hand.getWristTransform().getRotation());
  }
}

// If the user mirrors the image, we need to flip the second and third index for a face to keep
// a triangle's current clockwise / counter-clockwise direction.
void mirror(ArmModelGeometryMsg::Builder r) {
  for (auto localPoint : r.getVertices()) {
    mirror(localPoint);
  }

  for (auto normal : r.getNormals()) {
    mirror(normal);
  }

  for (auto triangleIndices : r.getIndices()) {
    const auto b = triangleIndices.getB();
    triangleIndices.setB(triangleIndices.getC());
    triangleIndices.setC(b);
  }
  ...
  // mirror the UVs so the texture continues to go from left to right
  for (auto UV : r.getUvs()) {
    UV.setU(1.0f - UV.getU());
  }
}

void mirror(FrameworkHandResponse::Builder r) {
  mirror(r.getLeftWristGeometry());
  mirror(r.getRightWristGeometry());
}
...
// Adjust the coordinate system in the response based on the coordinate system specified in the
// request.  This is performed on a per-frame basis for the HandResponse for variable data like
// vertices and normals.
void rewriteCoordinateSystemPostprocess(
  CameraCoordinates::Reader system, HandResponse::Builder response) {
  switch (system.getAxes()) {
    case CameraCoordinates::Axes::RIGHT_HANDED:
      makeRightHanded(response);
      break;
...
void rewriteCoordinateSystemFrameworkHand(
  CameraCoordinates::Reader system, FrameworkHandResponse::Builder response) {
  switch (system.getAxes()) {
    case CameraCoordinates::Axes::RIGHT_HANDED:
      makeRightHanded(response);
      break;
