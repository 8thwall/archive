...
//
// Test of the AgateDescriptor class functionality. For detailed benchmarking of
...
class AgateDescriptorTest : public ::testing::Test {
public:
  AgateDescriptorTest() : ctx(OffscreenGlContext::createRGBA8888Context()) {
...
TEST_F(AgateDescriptorTest, ExtractionAndProjection) {
  ScopeTimer rt("test");
  const FrameWithPoints &frame0 = frames[0];

  AgateDescriptor128 agate;
  auto vec0 = agate.extract(frame0.store().getFeatures<OrbFeature>()).clone();
  auto vecDup = agate.extract(frame0.store().getFeatures<OrbFeature>()).clone();

  auto distance = [](
                    const AgateDescriptor128::Descriptor &lhs,
                    const AgateDescriptor128::Descriptor &rhs) { return lhs.hammingDistance(rhs); };

  // The same feature extracted twice should have a distance of zero.
  EXPECT_EQ(0.0f, distance(vec0, vecDup));
  ...
  float dist01 = distance(vec0, vec1);
  float dist02 = distance(vec0, vec2);
  float dist12 = distance(vec1, vec2);
  float dist13 = distance(vec1, vec3);

  float dist07 = distance(vec0, vec7);
  float dist08 = distance(vec0, vec8);
  float dist09 = distance(vec0, vec9);

  float dist17 = distance(vec1, vec7);
  float dist18 = distance(vec1, vec8);
  float dist19 = distance(vec1, vec9);

  // The default AgateDescriptor32 is configured for classification. Matching images should have
  // lower distance than non-matching.
...
TEST_F(AgateDescriptorTest, MultiHistogramExtractionAndProjection) {
  ScopeTimer rt("test");
  const FrameWithPoints &frame0 = frames[0];

  AgateMultiDescriptor128 agate;
  auto vec0 = agate.extract(frame0.store().getFeatures<OrbFeature>()).clone();
  auto vecDup = agate.extract(frame0.store().getFeatures<OrbFeature>()).clone();
