...
- (id)initWithTitle:(char *)title withMessage:(char *)message {
  ...
  ShowAlertMessage(
    [[NSString alloc] initWithUTF8String:title], [[NSString alloc] initWithUTF8String:message]);
  return self;
}
...
- (id)initWithText:(char *)text withFilePaths:(char *)filePaths withSubject:(char *)subject {
  ...
  // if iPad
    ...
    UIPopoverController *popup =
      [[UIPopoverController alloc] initWithContentViewController:activity];
    [popup presentPopoverFromRect:CGRectMake(
                                    rootViewController.view.frame.size.width / 2,
                                    rootViewController.view.frame.size.height / 4,
                                    0,
                                    0)
...
- (BOOL)isStringValideBase64:(NSString *)string {
  ...
  NSUInteger regExMatches =
    [regEx numberOfMatchesInString:string options:0 range:NSMakeRange(0, [string length])];
...
void showSocialSharing(struct SocialSharingStruct *confStruct) {
  instance = [iOSNativeShare withText:confStruct->text
                        withFilePaths:confStruct->filePaths
