...
DetectionImage extractTargetFeatures(
  ...
  C8Log("[rainbow-detect] Reading image %s", targetFile.c_str());
...
void drawLiftedROICpu(
  ...
  float fullCols =
    (static_cast<float>(geom.srcCols) / geom.srcRows > .75f) ? geom.srcCols : .75f * geom.srcRows;
  float fullRows =
    (static_cast<float>(geom.srcCols) / geom.srcRows > .75f) ? geom.srcCols / .75f : geom.srcRows;

  RGBA8888PlanePixelBuffer pixCopyBuf(fullRows, fullCols);
  auto pixCopy = pixCopyBuf.pixels();
  fill(Color::BLACK, pixCopy);

  Vector<HPoint2> liftedTargetPixels;
  liftedTargetPixels.reserve(fullCols * fullRows);
  Vector<HPoint2> originalPixels;
  originalPixels.reserve(fullCols * fullRows);
  for (int i = 0; i < fullCols; i++) {
    for (int j = 0; j < fullRows; j++) {
      ...
      HPoint2 originalPointInClipSpace = isRotated
        ? HPoint2(
            originalPointInPixelSpace.y() / fullRows,
            1.f - (originalPointInPixelSpace.x() / fullCols))
        : HPoint2(
            originalPointInPixelSpace.x() / fullCols, originalPointInPixelSpace.y() / fullRows);
      HPoint2 pointInPixelSpace(
        originalPointInClipSpace.x() * fullCols, originalPointInClipSpace.y() * fullRows);
  ...
  // TODO(dat): Take as a parameter
  auto rainbowMetadata = buildRainbowMetadata(geom, 4463.f / 1436, 3212.f / 1436);
  mapRainbowToGeometryPoints(geom, rainbowMetadata, liftedTargetPixels, &modelPoints);
  mapToGeometryNormals(geom, liftedTargetPixels, &searchNormals);
...
void drawLiftedROIGpu(
  ...
  const String &name,
  Vector<String> *outPaths,
  float roiAspectRatio = 0.75f) {
  geom.radius += SHADER_ROI_RADIUS_INCREASE;
  geom.radiusBottom += SHADER_ROI_RADIUS_INCREASE;
  RGBA8888PlanePixelBuffer outBuf(searchPyramid.level(0).rows(), searchPyramid.level(0).cols());
  auto o = outBuf.pixels();
  GlFramebufferObject dest;
  dest.initialize(
    makeLinearRGBA8888Texture2D(o.cols(), o.rows()), GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0);
  ...
  int rotation = 0;
  if (searchTexture.width() > searchTexture.height()) {
    rotation = 90;
  }
  HPoint2 searchDims{
...
void drawMatchedPointsOnSearchImage(
  ...
  C8Log(
    "[rainbow-detect] Mapping geometry for matched points on search image numPoints=%d",
    targetPixels.size());
  Vector<HPoint3> pts;
  ...
  C8Log("[rainbow-detect] Done. Now writing out %s", name.c_str());
  writeOutputImage(pixCopy, name, outPaths);
}
...
void drawTargetFeaturesOnSearchImage(
  ...
  C8Log("[rainbow-detect] Generate all the pixels", "");
  ...
  C8Log(
    "[rainbow-detect] Compute pixel colors for target pix of size=%d x %d",
    targetPix.cols(),
    targetPix.rows());
  auto colors = pixelColors(targetPix, targetFeaturesInIm);
  C8Log("[rainbow-detect] Draw point cloud with back culling.", "");
  drawPointCloudWithBackCulling(intrinsics, extrinsics, pts, normals, colors, pixCopy);
  C8Log("[rainbow-detect] Done. Now writing out %s", name.c_str());
  writeOutputImage(pixCopy, name, outPaths);
}

// given a pose, prune the target points down to only those visible
DetectionImage pruneTargetPoints(
  const DetectionImage &targetImage, const CurvyImageGeometry &curvyGeom, const HMatrix &pose) {
  auto rainbowMetadata = buildRainbowMetadata(curvyGeom, 4463.f / 1436, 3212.f / 1436);

  const float visibleScale =
    1;  // simplistic scaling for now. The right approach should depends on the geometry curvature
  // only half of the cone is visible at a time. The angle with the center line is half that.
  const float visibleAngle = rainbowMetadata.theta / 4 * visibleScale;
  auto &targetPts = targetImage.framePoints();

  // Since we keep everything 4/3. Things are rotated 90 degree
  HVector2 midLine{1.f, 0.f};
  HPoint2 apex{curvyGeom.srcRows - rainbowMetadata.apex.y(), rainbowMetadata.apex.x()};
  // filter visible points. Doing this every frame is likely to cause high memory pressure
  // consider making matchers support masking
  TargetWithPoints visiblePoints(targetPts.intrinsic());
  visiblePoints.reserve(targetPts.points().size());
  auto targetPixels = targetPts.pixels();
  const auto &descriptors = targetPts.store().getFeatures<OrbFeature>();
  for (int i = 0; i < targetPts.points().size(); i++) {
    HVector2 pixelLine = line(apex, targetPixels[i]);
    float pixelAngle = std::acos(pixelLine.dot(midLine));
    if (pixelAngle <= visibleAngle) {
      const auto &targetPoint = targetPts.points()[i];
      visiblePoints.addImagePixelPoint(
        targetPixels[i],
        targetPoint.scale(),
        targetPoint.angle(),
        targetPoint.gravityAngle(),
        targetPts.store().getClone(i));
    }
  }
  return {
    targetImage.getName(),
    std::move(visiblePoints),
    targetImage.rotation(),
    targetImage.getGeometry(),
    targetImage.getFullGeometry(),
    targetImage.getFullLabelToCroppedLabelPose()};
}
...
void runCurvyDetect(
  ...
  DetectionImage targetPts = extractTargetFeatures(
    ...
    spec);
  C8Log(
    "[rainbow-detect] Extracted %d features from %s",
    targetPts.framePoints().points().size(),
    targetFile.c_str());

  // The geometry that our DetectionImage<OrbFeature> has is the one to be used for mapping
  CurvyImageGeometry curvyGeom = targetPts.getGeometry();
  C8Log("[rainbow-detect]  Input curvy spec: %s", spec.toString().c_str());
  C8Log("[rainbow-detect]  Computed geometry: %s", curvyGeom.toString().c_str());
  ...
  DetectionImage prunedTargetPts = pruneTargetPoints(targetPts, curvyGeom, HMatrixGen::i());
  C8Log("[rainbow-detect] Pruned target has %d features", targetPts.framePoints().points().size());
  drawFeaturesToFile(
    prunedTargetPts.framePoints(),
    targetLoader.gl().pyramid(),
    "pruned-target-features.png",
    outPaths);

  // Read the search image from disk.
  C8Log("[rainbow-detect] Reading image %s", searchFile.c_str());
  ...
  C8Log(
    "[rainbow-detect] Global matcher found %d matches with no score limit", globalMatches.size());
  ...
  globalMatches.clear();
  prunedTargetPts.globalMatcher().match(combinedPoints, 50, &globalMatches);
  C8Log("[rainbow-detect] Global matcher found %d matches", globalMatches.size());
  ...
  auto rainbowMetadata = buildRainbowMetadata(curvyGeom, 4463.f / 1436, 3212.f / 1436);
  getRainbowPointsOnGeometry(
    globalMatches,
    prunedTargetPts.framePoints().pixels(),
    curvyGeom,
    rainbowMetadata,
    &globalWorldPts);
  ...
  if (!found) {
    C8Log("[rainbow-detect] Target image was not found; we are done.");
    return;
  }
  ...
  C8Log(
    "[rainbow-detect] Found image with global search: %d (%d match inliers)",
    found,
  ...
  auto globalLocatedImage =
    locate(targetFile, globalPose, curvyGeom, prunedTargetPts, camPos, scale, searchK);
  C8Log("[rainbow-detect] Global pose %s", extrinsicToString(globalPose).c_str());

  C8Log(
    "[rainbow-detect] Drawing to size=%d x %d. Curvy src rows=%d cols=%d",
  ...
  drawRoi(searchTexture, gl.pyramid(), globalLocatedImage.roi, "region-of-interest.jpg", outPaths);
  ...
  auto featsRayInTargetView =
    projectSearchRayToTargetRay(prunedTargetPts, localFeatures, globalPose, curvyGeom);

  // Compute local matches in the space of the image target.
  Vector<PointMatch> localMatches;
  prunedTargetPts.localMatcher().useScaleFilter(true);
  prunedTargetPts.localMatcher().setDescriptorThreshold(64);
  prunedTargetPts.localMatcher().setRoiScale(true);
  prunedTargetPts.localMatcher().setRadius(0.1f);
  prunedTargetPts.localMatcher().findMatches(featsRayInTargetView, &localMatches);
  ...
  drawMatches(
    ...
    "local-matches.png",
    outPaths);
  C8Log("[rainbow-detect] Local matcher found %d matches", localMatches.size());
  ...
  getPointsOnGeometry(
    localMatches, prunedTargetPts.framePoints().pixels(), curvyGeom, &localWorldPts);
  ...
  if (!foundLocal) {
    C8Log("[rainbow-detect] Target image was not found in local search; we are done.");
    return;
  }
  ...
  C8Log(
    "[rainbow-detect] Found image with local search: %d (%d match inliers)",
    foundLocal,
  ...
  auto localLocatedImage =
    locate(targetFile, localPose, curvyGeom, prunedTargetPts, camPos, scale, searchK);
  drawTargetFeaturesOnSearchImage(
    targetLoader.gl().pyramid().level(0),
    gl.pyramid().level(0),
    prunedTargetPts.framePoints(),
...
int main(int argc, char *argv[]) {
  ...
  if (!std::empty(targetsJson)) {
    C8Log("[rainbow-detect]  JSON file target geometry input");
    ...
    C8Log(
      "[rainbow-detect]  Looking in JSON file %s for target with key: \"%s\"",
      targetsJson.c_str(),
      targetName.c_str());
    auto targetJson = readJsonFile(targetsJson, targetName);
    spec = curvySpecFromTargetJson(targetJson);
  } else {
    C8Log("[rainbow-detect]  Command line target geometry input");
    ...
    if (result.count("arc") == 0) {
      C8Log(
        "[rainbow-detect]  You have to provide the arc of the curve image target between (0,1]",
        "");
      return -1;
    }
    if (result.count("imagePath") == 0) {
      C8Log("[rainbow-detect]  You have to specify the texture to visualize the features from", "");
      return -1;
    }
    if (result.count("searchPath") == 0) {
      C8Log(
        "[rainbow-detect]  You have to specify the search path (an image from the camera feed", "");
      return -1;
    }
  ...
  Vector<String> outPaths;
  {
    ...
    runCurvyDetect(imagePath, spec, searchPath, &outPaths);
  }
  // C8Log("------------------\nTiming summary:");
